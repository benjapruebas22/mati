...
def register_sst_routes(app, bp, get_db, ensure_cols, ensure_sedes_mpd_cols, cal_colors, ensure_auth_tables, default_redirect_for_role=None):
    ...
    SEDE_ESTADO_VARS = ['relevamiento', 'obra_terminada', 'matafuegos_recarga', 'carteleria', 'luces_emergencia', 'plano_evac', 'orden_limpieza', 'senalizacion', 'accesibilidad', 'riesgo_electrico']
    SEDE_ESTADO_LABELS = {'relevamiento': 'Relevamiento', 'obra_terminada': 'Obra terminada', 'matafuegos_recarga': 'Matafuegos recarga', 'carteleria': 'Carteleria', 'luces_emergencia': 'Luces emergencia', 'plano_evac': 'Plano evacuacion', 'orden_limpieza': 'Orden / limpieza', 'senalizacion': 'Senalizacion', 'accesibilidad': 'Accesibilidad', 'riesgo_electrico': 'Riesgo electrico'}
    DOCUMENTOS_TIPOS = ['informe', 'protocolo', 'instruccion', 'acta', 'nota', 'documento_general']
    DOCUMENTOS_ESTADOS = ['borrador', 'emitido', 'enviado', 'cerrado', 'archivado']
    DOCUMENTOS_DESTINOS = ['Defensora General', 'Administrador General', 'Archivo institucional', 'Interno Intendencia']
    ...
    def delete_eventos_fuente(con, fuente, ref, tipos):
        ...
        q = ','.join(['?'] * len(tipos))
        con.execute(f'DELETE FROM eventos WHERE fuente=? AND ref=? AND tipo IN ({q})', (fuente, ref, *tipos))

    def add_evento(fecha, titulo, detalle='', color='#3B82F6', fuente='sistema', ref_id=None):
        if not fecha:
            return
        con = get_db()
        con.execute('\n            INSERT INTO eventos(fecha, titulo, detalle, color, fuente, ref_id)\n            VALUES (?, ?, ?, ?, ?, ?)\n        ', (fecha, titulo[:120], (detalle or '')[:500], color, fuente, str(ref_id) if ref_id is not None else None))
        con.commit()
        con.close()
    ...
    def rebuild_eventos_limpieza_sede():
        ...
        for fila in filas:
            ...
            add_evento_tipo(fecha=fecha_ini, tipo='limpieza_sede', titulo='Limpieza asignada', detalle=detalle, fuente='limpieza_sede', ref_id=ref_id)
        con.commit()
        con.close()
    ...
    def _guardias_pasivas_plan_2026():
        return [{'mes': 'enero', 'chofer': 'Ignacio Baroni'}, {'mes': 'febrero', 'chofer': 'Mauro Vea Murguia'}, {'mes': 'marzo', 'chofer': 'Emiliano Perez de la Puente'}, {'mes': 'abril', 'chofer': 'Ignacio Baroni'}, {'mes': 'mayo', 'chofer': 'Mauro Vea Murguia'}, {'mes': 'junio', 'chofer': 'Jorge Corbacho'}, {'mes': 'julio', 'chofer': 'Francisco Savio / Manuel Flores'}, {'mes': 'agosto', 'chofer': 'Matias Calderari'}, {'mes': 'septiembre', 'chofer': 'Emiliano Perez de la Puente'}, {'mes': 'octubre', 'chofer': 'Ignacio Baroni'}, {'mes': 'noviembre', 'chofer': 'Mauro Vea Murguia'}, {'mes': 'diciembre', 'chofer': 'Jorge Corbacho'}]
    ...
    def _ensure_dashboard_novedades_obra_table(con):
        ...
        if 'responsable' not in cols:
            try:
                ...
                pass
        con.commit()
    NVD_TIPO_SUBTIPOS = {'Licencia': ['Particular', 'Compensatorio', 'Horas extra', 'Cambio de horario', 'Otro'], 'Pedido de materiales': ['Pintura', 'Durlock', 'Construccion', 'Plomeria', 'Albanileria', 'Aire acondicionado', 'Desinfeccion', 'Humedad', 'Limpieza', 'Electricidad', 'Mobiliario', 'Herreria', 'Mudanza', 'Otros'], 'Uso de salon': ['Reserva', 'Cambio de fecha', 'Armado de mesas', 'Cantidad de personas'], 'Reclamo / mantenimiento': ['Iluminacion', 'Agua', 'Bano', 'Electricidad', 'Cerradura', 'Humedad', 'Mobiliario', 'Limpieza', 'Otro'], 'Gestion operativa': ['Cargar horario especial', 'Cargar por sistema', 'Pedir por sistema', 'Solicitud especial', 'Reunion / recordar', 'Te busco / coordinacion', 'Otro'], 'Vehiculo': ['Guardar vehiculo (patente)', 'Mecanico / necesita arreglo', 'Necesita arreglo urgente', 'Necesita reparacion', 'Carga por sistema', 'Otro'], 'Vehiculos': ['Guardar vehiculo (patente)', 'Mecanico / necesita arreglo', 'Necesita arreglo urgente', 'Necesita reparacion', 'Carga por sistema', 'Otro'], 'Aviso general': ['Novedad diaria', 'Reorganizacion', 'Cambio operativo', 'Otro'], 'Otro': ['General']}
    ...
    def _ensure_novedades_catalogo_table(con):
        ...
        for name, sql_type in (('grupo', 'TEXT'), ('tipo_ref', "TEXT DEFAULT ''"), ('valor', 'TEXT'), ('activo', 'INTEGER DEFAULT 1'), ('creado_en', 'TEXT')):
            if name not in cols:
                try:
    ...
    def _ensure_novedades_diarias_table(con):
        ...
        for name, sql_type in (('hora', 'TEXT'), ('agente', 'TEXT'), ('sede_codigo', 'TEXT'), ('subtipo', 'TEXT'), ('observacion', 'TEXT'), ('estado', "TEXT DEFAULT 'Informado'"), ('creado_en', 'TEXT'), ('actualizado_en', 'TEXT')):
            if name not in cols:
                try:
    ...
    def _dashboard_vehiculos_simple(con, fecha_iso):
        ...
        try:
            ...
            for r in rows:
                ...
                out.append({'patente': pat, 'codigo': alias or pat, 'estado': estado})
        except Exception:
            pass
        return out

    def _dashboard_alertas_criticas(data):
        kws = ('venc', 'vence', 'vtv', 'rtv', 'seguro', 'carnet', 'matafuego', 'service', 'servicio', 'licencia')
    ...
    def _dashboard_operativo_data():
        ...
        data = {'vehiculos': {'donut': {'enUso': 0, 'guardados': 0, 'pendientesCierre': 0, 'noDisponibles': 0, 'total': 0}, 'finJornada': {'pendientesCierre': 0, 'ok': True}, 'topAsignacion': [], 'ultimosViajesLargos': [], 'manualMovimientos': [], 'catalogos': {'vehiculos': [], 'choferes': [], 'destinos': []}, 'proceso': {'responsableTactico': 'Ignacio Baroni'}}, 'materiales': {'internosPendientes': 0, 'enviadosCompra': 0, 'entregasPendientesCierre': 0}, 'obras': {'enEjecucionHoy': 0, 'cerradasHoy': 0, 'urgenciasExternas': 0, 'novedadesHoy': [], 'novedadesCount': 0}, 'horarios': {'pendienteMail': 0, 'enviadosHoy': 0, 'turnosChoferesSinAsignarMes': 0}, 'personal': {'distribucion': [], 'totalAsignado': 0, 'sedesSinPersonal': 0, 'snapshot': 'hoy'}, 'asignacionDia': {'licenciasDia': [], 'compensatoriosActivos': [], 'turnoMesChoferes': [], 'turnoSemanaVespertino': [], 'choferes': [], 'turnosCfg': {'mesMensual': '', 'choferMensual': '', 'semanaDesde': '', 'semanaHasta': '', 'choferSemanal': ''}, 'guardiasPasivas': {'plan2026': _guardias_pasivas_plan_2026(), 'mensual': {'estado': 'pendiente', 'texto': 'Pendiente de aceptacion', 'aceptadoEn': '', 'chofer': '', 'periodo': ''}, 'semanal': {'estado': 'pendiente', 'texto': 'Pendiente de aceptacion', 'aceptadoEn': '', 'chofer': '', 'periodo': ''}}, 'limpiezaTurnosSede': [], 'gruposLimpieza': [], 'rotacionActiva': {'mes': '', 'proximaFecha': '', 'filas': [], 'refuerzoTexto': 'El grupo asignado a San Martin 137 cubre Alto Comedero y Palpala cuando Intendencia lo disponga.'}, 'instructivoRotacion': ['La rotacion es mensual y automatica.', 'Cada grupo rota por todas las sedes en ciclos de 4 meses.', 'Todos los agentes pasan por todas las sedes (criterio de equidad).', 'La sede San Martin 137, por su menor volumen operativo, actua como sede base de refuerzo territorial.', 'El grupo asignado a San Martin 137 cubre Alto Comedero y Palpala cuando Intendencia lo disponga.', 'Las licencias activan cobertura interna dentro del grupo.', 'El sistema prioriza equilibrio de carga laboral y justicia operativa.']}, 'fechaHoy': today_iso, 'indicadores2026': {'kmPorVehiculo': [], 'kmPorChofer': [], 'totalKm': 0.0}, 'calendario': {'fechaSel': today_iso, 'diasConEventos': [], 'diasMeta': {}, 'hoy': [], 'proximos7': [], 'resumen': {'eventosHoy': 0, 'eventos7': 0, 'alertasCriticas': 0}}, 'recordatorios': [], 'licenciasHoy': 0, 'sedeEstado': {'promedioPct': 0, 'items': [], 'variables': list(SEDE_ESTADO_VARS)}}
        ...
        if _table_exists(con, 'vehiculos'):
            ...
            if 'activo' in veh_cols:
                try:
                    row_nd = con.execute('SELECT COUNT(*) AS n FROM vehiculos WHERE COALESCE(activo, 1) = 0').fetchone()
            ...
            data['vehiculos']['donut'] = {'enUso': int(en_uso), 'guardados': int(guardados), 'pendientesCierre': int(pendientes), 'noDisponibles': int(no_disponibles), 'total': int(total)}
            data['vehiculos']['finJornada'] = {'pendientesCierre': int(pendientes), 'ok': int(pendientes) == 0}
            if 'patente' in veh_cols:
                ...
                for r in rows_top:
                    ...
                    items.append({'patente': patente or '—', 'alias': alias or '—', 'ubicacion': ubicacion, 'kmSemana': km_semana, 'kmMes': km_mes, 'estado': estado})
                estado_order = {'Disponible': 0, 'Pendiente cierre': 1, 'En uso': 2, 'No disponible': 3}
        ...
        if _table_exists(con, 'viajes') and {'fecha', 'patente'}.issubset(viajes_cols):
            ...
            try:
                ...
                for r in rows_mov:
                    ...
                    if not pat or pat in seen_pat:
                        continue
                    seen_pat.add(pat)
                    mov.append({'patente': pat, 'chofer': (_row_value(r, 'chofer', '') or '').strip() or '-', 'destino': (_row_value(r, 'destino', '') or '').strip() or '-', 'fecha': (_row_value(r, 'fecha', '') or '').strip(), 'horaSalida': (_row_value(r, 'hora_salida', '') or '').strip(), 'horaRegresoEstimada': (_row_value(r, 'hora_regreso_estimada', '') or '').strip(), 'estado': (_row_value(r, 'estado', '') or '').strip()})
            ...
            try:
                ...
                data['vehiculos']['ultimosViajesLargos'] = [{'fecha': (_row_value(r, 'fecha', '') or '').strip(), 'chofer': (_row_value(r, 'chofer', '') or '').strip() or '-', 'destino': (_row_value(r, 'destino', '') or '').strip() or '-', 'vehiculo': (_row_value(r, 'patente', '') or '').strip() or '-', 'horaSalida': (_row_value(r, 'hora_salida', '') or '').strip(), 'horaRegresoEstimada': (_row_value(r, 'hora_regreso_estimada', '') or '').strip()} for r in rows_viajes]
            except Exception:
                pass
        tabla_pedidos = None
        ...
        if _table_exists(con, 'obras_sede'):
            ...
            if f_fin:
                try:
                    row = con.execute('SELECT COUNT(*) AS n FROM obras_sede WHERE fecha_fin_real IS NOT NULL AND date(fecha_fin_real) = date(?)', (today_iso,)).fetchone()
        ...
        if _table_exists(con, 'agentes_licencias'):
            ...
            if {'fecha_desde', 'fecha_hasta'}.issubset(cols_lic):
                ...
                try:
                    ...
                    data['asignacionDia']['licenciasDia'] = [{'agente': (_row_value(r, 'agente', '-') or '-').strip(), 'tipo': (_row_value(r, 'tipo', '') or '').strip(), 'desde': (_row_value(r, 'desde', '') or '').strip(), 'hasta': (_row_value(r, 'hasta', '') or '').strip()} for r in rows_lic]
                except Exception:
                    pass
        if _table_exists(con, 'agentes_compensatorios_mov'):
            try:
                ...
                data['asignacionDia']['compensatoriosActivos'] = [{'agente': (_row_value(r, 'agente', '-') or '-').strip(), 'desde': (_row_value(r, 'desde', '') or '').strip(), 'hasta': (_row_value(r, 'hasta', '') or '').strip(), 'tipo': (_row_value(r, 'tipo', '') or '').strip()} for r in rows_comp]
            except Exception:
                pass
        if _table_exists(con, 'agentes_intendencia'):
            try:
                choferes_permitidos = ('Emiliano P de la Puente', 'Emiliano Perez de la Puente', 'Ignacio Baroni', 'Mauro Vea Murguia', 'Luis Cardozo')
        ...
        try:
            ...
            data['asignacionDia']['turnosCfg'] = {'mesMensual': (_row_value(row_cfg, 'mes_mensual', '') or '').strip(), 'choferMensual': (_row_value(row_cfg, 'chofer_mensual', '') or '').strip(), 'semanaDesde': (_row_value(row_cfg, 'semana_desde', '') or '').strip(), 'semanaHasta': (_row_value(row_cfg, 'semana_hasta', '') or '').strip(), 'choferSemanal': (_row_value(row_cfg, 'chofer_semanal', '') or '').strip()}
            ...
            data['asignacionDia']['guardiasPasivas']['mensual'] = {'estado': 'aceptada' if ack_m else 'pendiente', 'texto': 'Aceptada' if ack_m else 'Pendiente de aceptacion', 'aceptadoEn': (_row_value(ack_m, 'aceptado_en', '') if ack_m else '') or '', 'chofer': chofer_m, 'periodo': periodo_m, 'mes': mes_cfg}
            data['asignacionDia']['guardiasPasivas']['semanal'] = {'estado': 'aceptada' if ack_s else 'pendiente', 'texto': 'Aceptada' if ack_s else 'Pendiente de aceptacion', 'aceptadoEn': (_row_value(ack_s, 'aceptado_en', '') if ack_s else '') or '', 'chofer': chofer_s, 'periodo': periodo_s, 'desde': semanal_desde, 'hasta': semanal_hasta}
        except Exception:
            pass
        try:
        ...
        if _table_exists(con, 'dashboard_sede_estado'):
            ...
            try:
                ...
                for r in rows_est:
                    ...
                    items.append({'sede': (_row_value(r, 'sede_codigo', '') or '').strip() or '-', 'puntos': pts, 'pct': pct})
        ...
        if _table_exists(con, 'sedes_limpieza'):
            ...
            try:
                ...
                data['asignacionDia']['limpiezaTurnosSede'] = [{'sede': (_row_value(r, 'sede', '') or '').strip(), 'turno': (_row_value(r, 'turno', '') or '').strip() or 's/d', 'responsable': (_row_value(r, 'responsable', '') or '').strip() or '-'} for r in rows_turn if (_row_value(r, 'sede', '') or '').strip()]
            except Exception:
                pass
            if _table_exists(con, 'sedes_mpd'):
                try:
        ...
        if _table_exists(con, 'agentes_intendencia'):
            try:
                ...
                data['asignacionDia']['gruposLimpieza'] = [{'grupo': f'GR{idx + 1}', 'agentes': grupos[idx]} for idx in range(4)]
            except Exception:
                pass
        try:
            ...
            for idx_sede, sede in enumerate(sedes_ciclo):
                ...
                filas.append({'sede': sede, 'grupo': grp, 'matutino': ags[0] if len(ags) > 0 else '-', 'vespertino': ags[1] if len(ags) > 1 else '-'})
            try:
        ...
        if _table_exists(con, 'viajes') and 'fecha' in viajes_cols:
            ...
            try:
                ...
                data['indicadores2026']['kmPorVehiculo'] = [{'label': (_row_value(r, 'patente', '') or '').strip() or '-', 'km': round(float(_row_value(r, 'km', 0) or 0), 1)} for r in rows]
            except Exception:
                pass
            ...
            try:
                ...
                data['indicadores2026']['kmPorChofer'] = [{'label': (_row_value(r, 'chofer', '') or '').strip() or '-', 'km': round(float(_row_value(r, 'km', 0) or 0), 1)} for r in rows]
            except Exception:
                pass
            data['indicadores2026']['totalKm'] = round(sum((float(x.get('km', 0) or 0) for x in data['indicadores2026']['kmPorVehiculo'])), 1)
        if _table_exists(con, 'eventos'):
            ...
            if {'fecha', 'titulo'}.issubset(cols_ev):
                try:
                    ...
                    data['calendario']['diasConEventos'] = [{'fecha': (_row_value(r, 'fecha', '') or '').strip(), 'n': int(_row_value(r, 'n', 0) or 0)} for r in rows_mes]
                    ...
                    for r in rows_meta:
                        ...
                        txt = ((_row_value(r, 'titulo', '') or '') + ' ' + (_row_value(r, 'detalle', '') or '')).lower()
                        if any((k in txt for k in ('urgente', 'venc', 'crit', 'pendiente'))) or src in ('obras', 'seguridad'):
                            meta[f]['critica'] = True
                    data['calendario']['diasMeta'] = {k: {'colores': sorted(list(v['colores']))[:3], 'critica': bool(v['critica'])} for k, v in meta.items()}
                except Exception:
                    pass
                try:
                    rows_hoy = con.execute('\n                        SELECT fecha, titulo, detalle, fuente, color\n                        FROM eventos\n                        WHERE date(fecha) = date(?)\n                        ORDER BY id DESC\n                        LIMIT 8\n                    ', (today_iso,)).fetchall()
                    data['calendario']['hoy'] = [{'fecha': (_row_value(r, 'fecha', '') or '').strip(), 'titulo': (_row_value(r, 'titulo', '') or '').strip(), 'detalle': (_row_value(r, 'detalle', '') or '').strip(), 'fuente': (_row_value(r, 'fuente', '') or '').strip(), 'color': (_row_value(r, 'color', '') or '').strip()} for r in rows_hoy]
                except Exception:
                    pass
                try:
                    rows_7 = con.execute("\n                        SELECT fecha, titulo, detalle, fuente, color\n                        FROM eventos\n                        WHERE date(fecha) >= date(?)\n                          AND date(fecha) <= date(?,'+6 day')\n                        ORDER BY date(fecha), id\n                        LIMIT 20\n                    ", (today_iso, today_iso)).fetchall()
                    data['calendario']['proximos7'] = [{'fecha': (_row_value(r, 'fecha', '') or '').strip(), 'titulo': (_row_value(r, 'titulo', '') or '').strip(), 'detalle': (_row_value(r, 'detalle', '') or '').strip(), 'fuente': (_row_value(r, 'fuente', '') or '').strip(), 'color': (_row_value(r, 'color', '') or '').strip()} for r in rows_7]
                except Exception:
                    pass
                ...
                for ev in data['calendario']['proximos7']:
                    ...
                    if any((k in txt for k in ('urgente', 'venc', 'crit', 'pendiente'))) or any((c in col for c in ('#dc2626', '#ef4444', '#b91c1c'))):
                        crit += 1
        ...
        if _table_exists(con, 'calendario_pedidos'):
            try:
                ...
                data['calendario']['diasConEventos'] = [{'fecha': f, 'n': by_fecha[f]} for f in sorted(by_fecha.keys()) if f]
                ...
                for r in rows_hoy_p:
                    ...
                    if sede:
                        titulo += f' ({sede})'
                    data['calendario']['hoy'].append({'fecha': (_row_value(r, 'fecha', '') or '').strip(), 'titulo': titulo, 'detalle': (det or sol or '').strip(), 'estado': est or 'Pedir', 'fuente': 'calendario_pedidos', 'color': '#0ea5e9'})
                ...
                for r in rows_7_p:
                    ...
                    if sede:
                        titulo += f' ({sede})'
                    data['calendario']['proximos7'].append({'fecha': (_row_value(r, 'fecha', '') or '').strip(), 'titulo': titulo, 'detalle': (det or sol or '').strip(), 'estado': est or 'Pedir', 'fuente': 'calendario_pedidos', 'color': '#0ea5e9'})
                ...
                row_p_hoy = con.execute('SELECT COUNT(*) AS n FROM calendario_pedidos WHERE date(fecha) = date(?)', (today_iso,)).fetchone()
        ...
        if int(data['horarios']['turnosChoferesSinAsignarMes'] or 0) > 0:
            data['recordatorios'].append(f"{int(data['horarios']['turnosChoferesSinAsignarMes'])} turnos de chofer sin asignar (proximo mes)")
        if int(data['horarios']['pendienteMail'] or 0) > 0:
            data['recordatorios'].append(f"{int(data['horarios']['pendienteMail'])} mails de horario especial pendientes")
        if int(data['materiales']['internosPendientes'] or 0) > 0:
            data['recordatorios'].append(f"{int(data['materiales']['internosPendientes'])} pedidos internos de materiales")
        try:
            ...
            data['vehiculos']['proceso']['responsableTactico'] = (_row_value(row_vcfg, 'responsable_tactico', 'Ignacio Baroni') or 'Ignacio Baroni').strip()
            if _table_exists(con, 'vehiculos'):
                vrows = con.execute("\n                    SELECT\n                        COALESCE(patente, '') AS patente,\n                        COALESCE(codigo_interno, '') AS alias\n                    FROM vehiculos\n                    WHERE COALESCE(activo, 1) = 1\n                    ORDER BY codigo_interno, patente\n                ").fetchall()
                data['vehiculos']['catalogos']['vehiculos'] = [{'value': (_row_value(r, 'patente', '') or '').strip(), 'label': ((_row_value(r, 'alias', '') or '').strip() + ' - ' + (_row_value(r, 'patente', '') or '').strip()).strip(' -')} for r in vrows if (_row_value(r, 'patente', '') or '').strip()]
            if _table_exists(con, 'agentes_intendencia'):
                crows = con.execute("\n                    SELECT COALESCE(agente, '') AS agente\n                    FROM agentes_intendencia\n                    WHERE COALESCE(activo, 1) = 1\n                      AND LOWER(COALESCE(rubro, '')) = 'choferes'\n                    ORDER BY agente\n                ").fetchall()
                data['vehiculos']['catalogos']['choferes'] = [(_row_value(r, 'agente', '') or '').strip() for r in crows if (_row_value(r, 'agente', '') or '').strip()]
            if _table_exists(con, 'destinos'):
                drows = con.execute("\n                    SELECT COALESCE(nombre, '') AS nombre\n                    FROM destinos\n                    WHERE COALESCE(activo, 1) = 1\n                    ORDER BY nombre\n                ").fetchall()
                data['vehiculos']['catalogos']['destinos'] = [(_row_value(r, 'nombre', '') or '').strip() for r in drows if (_row_value(r, 'nombre', '') or '').strip()]
        except Exception:
            pass
        try:
            ...
            data['vehiculos']['manualMovimientos'] = [{'id': int(_row_value(r, 'id', 0) or 0), 'fecha': (_row_value(r, 'fecha', '') or '').strip(), 'vehiculo': (_row_value(r, 'vehiculo', '') or '').strip(), 'chofer': (_row_value(r, 'chofer', '') or '').strip(), 'destino': (_row_value(r, 'destino', '') or '').strip(), 'horaSalida': (_row_value(r, 'hora_salida', '') or '').strip(), 'horaRegresoEstimada': (_row_value(r, 'hora_regreso_estimada', '') or '').strip(), 'estado': (_row_value(r, 'estado', 'En uso') or 'En uso').strip(), 'combustible': (_row_value(r, 'combustible', '') or '').strip(), 'materiales': (_row_value(r, 'materiales', '') or '').strip(), 'agenteTraslado': (_row_value(r, 'agente_traslado', '') or '').strip(), 'observaciones': (_row_value(r, 'observaciones', '') or '').strip()} for r in rows_vm]
        except Exception:
            pass
        try:
            ...
            data['obras']['novedadesHoy'] = [{'id': int(_row_value(r, 'id', 0) or 0), 'fecha': (_row_value(r, 'fecha', '') or '').strip(), 'texto': (_row_value(r, 'texto', '') or '').strip(), 'urgente': int(_row_value(r, 'urgente', 0) or 0), 'tipo': (_row_value(r, 'tipo', 'novedad') or 'novedad').strip(), 'estado': (_row_value(r, 'estado', 'nuevo') or 'nuevo').strip(), 'responsable': (_row_value(r, 'responsable', '') or '').strip()} for r in rows_nov if (_row_value(r, 'texto', '') or '').strip()]
    ...
    def _vehiculos_cards_from_base(base, con):
        ...
        if _table_exists(con, 'vehiculos'):
            ...
            for r in rows:
                ...
                if not pat:
                    continue
                veh_lookup[pat] = {'tipo': (_row_value(r, 'tipo', '') or '').strip(), 'combustible': (_row_value(r, 'combustible', '') or '').strip(), 'base': (_row_value(r, 'base_ciudad', '') or '').strip(), 'lugar': (_row_value(r, 'lugar_reservado', '') or '').strip(), 'activo': int(_row_value(r, 'activo', 1) or 1)}
        ...
        if not base_items and veh_lookup:
            for pat, v in list(veh_lookup.items())[:6]:
                base_items.append({'patente': pat, 'alias': '-', 'ubicacion': v.get('base') or v.get('lugar') or '-', 'kmSemana': '-', 'kmMes': '-', 'estado': 'Disponible' if v.get('activo', 1) else 'No disponible'})
        ...
        for item in base_items[:8]:
            ...
            vehiculos_cards.append({'patente': pat, 'estado': estado, 'combustible': v.get('combustible') or '-', 'km': km_txt, 'sede': item.get('ubicacion') or v.get('base') or v.get('lugar') or '-', 'uso': v.get('tipo') or '-', 'bar': _bar_pct(estado)})
    ...
    def _obras_sedes_resumen(con):
        ...
        if _table_exists(con, 'sedes_mpd'):
            ...
            for r in rows:
                ...
                if not cod:
                    continue
                sedes_lookup[cod] = {'nombre': (_row_value(r, 'nombre', '') or '').strip(), 'direccion': (_row_value(r, 'direccion', '') or '').strip()}
        if _table_exists(con, 'obras_sede'):
            cols_obras = _table_cols(con, 'obras_sede')
            if 'codigo_sede' in cols_obras:
                ...
                for idx, r in enumerate(rows[:8]):
                    ...
                    obras_sedes.append({'codigo': sede, 'label': label, 'pct': pct, 'n': n, 'color': color})
                    start = acc
                    end = acc + pct
    ...
    @bp.route('/dashboard/sgi', endpoint='sgi_home')
    def sgi_home():
        ...
        sgi = {'vehiculos': {'total': 0, 'estado': 'Sin datos', 'fuera_servicio': 0}, 'obras': {'pendientes': 0, 'en_curso': 0, 'alta_prioridad': 0}, 'sedes': {'total': 20, 'con_alertas': 0}, 'seguimiento': {'estado': 'Activo', 'novedades_hoy': 0}, 'alertas': {'estado': 'Monitoreo', 'criticas': 0}, 'alcance': {'sigla': 'MPD', 'texto': 'Cobertura operativa institucional'}}
        base = {}
        try:
        ...
        if _to_int(sgi['sedes'].get('con_alertas'), 0) <= 0 and _to_int(sgi['alertas'].get('criticas'), 0) > 0:
            sgi['sedes']['con_alertas'] = min(_to_int(sgi['alertas'].get('criticas'), 0), max(_to_int(sgi['sedes'].get('total'), 20), 0))
        vehiculos_cards = []
        obras_sedes = []
        obras_total = 0
        obras_donut = ''
        ...
        try:
            ...
            try:
                ...
                pass
        return render_template('sgi_home.html', sgi=sgi, vehiculos_cards=vehiculos_cards, obras_sedes=obras_sedes, obras_total=obras_total, obras_donut=obras_donut)

    @bp.route('/dashboard/alta-direccion', endpoint='dashboard_ejecutivo')
    def dashboard_ejecutivo():
        ...
        try:
            ...
            if _table_exists(con, 'sedes_mpd'):
                ...
                for s in rows_sedes:
                    ...
                    if depositos_m is not None:
                        ...
                        try:
                            depositos_kpi = con.execute('SELECT COUNT(*) AS c FROM sedes_depositos WHERE codigo_sede = ?', (cod,)).fetchone()['c']
                        except Exception:
                    ...
                    if idx_general is None:
                        ...
                        idx_class = 'bad'
                    sedes_cards.append({'codigo': cod, 'nombre': _row_value(s, 'nombre', '') or '', 'ciudad': _row_value(s, 'ciudad', '') or '', 'direccion': _row_value(s, 'direccion', '') or '', 'personal': _safe_int(_row_value(per_kpi, 'personas', 0)), 'puestos': puestos_trabajo, 'depositos': depositos_kpi or 0, 'ocupacion_pct': ocupacion_pct, 'm2pp': m2pp, 'ppo': ppo, 'idx_general': idx_general, 'idx_class': idx_class})
            ...
            if _table_exists(con, 'obras_sede'):
                ...
                for r in rows_o:
                    top_obras.append({'sede': (_row_value(r, 'codigo_sede', '') or '').strip(), 'titulo': (_row_value(r, 'titulo', '') or '').strip(), 'tipo': (_row_value(r, 'tipo', '') or '').strip(), 'prioridad': (_row_value(r, 'prioridad', '') or '').strip(), 'estado': (_row_value(r, 'estado', '') or '').strip()})
            ...
            uso_general = {'en_uso': int(donut.get('enUso') or 0), 'guardados': int(donut.get('guardados') or 0), 'pendientes': int(donut.get('pendientesCierre') or 0), 'no_disp': int(donut.get('noDisponibles') or 0), 'total': veh_total}
            sst_top = sorted(items_sst, key=lambda x: x.get('pct', 0), reverse=True)[:6]
        finally:
            con.close()
        return render_template('dashboard_ejecutivo.html', sedes_activas=sedes_activas, obras_en_curso=obras_en_curso, pendientes_criticos=pendientes_criticos, vehiculos_operativos=veh_operativos, sst_avance=sst_avg, sst_estado=sst_estado, sst_estado_cls=sst_estado_cls, sst_etapa=sst_etapa, sedes_cards=sedes_cards[:8], sedes_total=len(sedes_cards), obras_avance_pct=obras_avance_pct, obras_total=obras_total, obras_sedes=obras_sedes, obras_donut=obras_donut, top_obras=top_obras, vehiculos_cards=vehiculos_cards, total_km=total_km, uso_general=uso_general, sst_top=sst_top)

    @bp.route('/dashboard/sgi/documentacion', endpoint='sgi_documentacion')
    def sgi_documentacion():
        return render_template('sgi_documentacion.html')

    @bp.route('/dashboard/sgi/documentacion/informes', methods=['GET', 'POST'], endpoint='sgi_documentacion_informes')
    def sgi_documentacion_informes():
        ...
        def _sync_documento_relaciones(cur, doc_id, limpiar_previas=False):
            ...
            for sede_codigo in sorted(set([x.strip().upper() for x in request.form.getlist('sedes_codigos') if str(x).strip()])):
                cur.execute('INSERT OR IGNORE INTO documentos_sedes(id_documento, sede_codigo) VALUES (?, ?)', (doc_id, sede_codigo))
            agentes_ids = []
            ...
            for aid in sorted(set(agentes_ids)):
                cur.execute('INSERT OR IGNORE INTO documentos_agentes(id_documento, id_agente) VALUES (?, ?)', (doc_id, aid))
            for patente in sorted(set([x.strip().upper() for x in request.form.getlist('vehiculos_patentes') if str(x).strip()])):
                cur.execute('INSERT OR IGNORE INTO documentos_vehiculos(id_documento, patente) VALUES (?, ?)', (doc_id, patente))
            ...
            if sst_tipo_evento or sst_evento_id is not None:
                cur.execute('INSERT OR IGNORE INTO documentos_sst(id_documento, tipo_evento, id_evento) VALUES (?, ?, ?)', (doc_id, sst_tipo_evento or None, sst_evento_id))
            for tag in _split_doc_tags(request.form.get('tags') or ''):
                cur.execute('INSERT OR IGNORE INTO documentos_tags(id_documento, tag) VALUES (?, ?)', (doc_id, tag))
            destinos = []
            ...
            for d in sorted(set(destinos)):
                cur.execute('INSERT OR IGNORE INTO documentos_destino(id_documento, destino) VALUES (?, ?)', (doc_id, d))
        if request.method == 'POST':
            ...
            if action == 'delete':
                ...
                con.close()
                return redirect(url_for('sst.sgi_documentacion_informes'))
            ...
            if not titulo:
                ...
                if action == 'edit':
                    ...
                    if doc_id <= 0:
                        ...
                        cur.execute('\n                            UPDATE documentos\n                            SET titulo = ?,\n                                tipo_documento = ?,\n                                descripcion = ?,\n                                fecha = ?,\n                                autor = ?,\n                                archivo_url = ?,\n                                estado = ?\n                            WHERE id_documento = ?\n                            ', (titulo, tipo_documento, descripcion or None, fecha or None, autor or None, archivo_url or None, estado, doc_id))
                        existe = con.execute('SELECT 1 FROM documentos WHERE id_documento = ?', (doc_id,)).fetchone()
                        if existe:
                    ...
                    cur.execute('\n                        INSERT INTO documentos(titulo, tipo_documento, descripcion, fecha, autor, archivo_url, estado)\n                        VALUES (?, ?, ?, ?, ?, ?, ?)\n                        ', (titulo, tipo_documento, descripcion or None, fecha or None, autor or None, archivo_url or None, estado))
        ...
        con.close()
        return render_template('sgi_documentacion_informes.html', docs=docs, sedes=sedes, agentes=agentes, vehiculos=vehiculos, sst_eventos=sst_eventos, tipos_documento=DOCUMENTOS_TIPOS, estados_documento=DOCUMENTOS_ESTADOS, destinos_documento=DOCUMENTOS_DESTINOS, tags_disponibles=tags_disponibles, doc_editar=doc_editar, q_texto=q_texto, q_tipo=q_tipo, q_estado=q_estado, q_sede=q_sede, q_agente=q_agente, q_vehiculo=q_vehiculo, q_destino=q_destino, q_tag=q_tag)

    @bp.route('/dashboard/materiales-historial', endpoint='dashboard_materiales_historial')
    def dashboard_materiales_historial():
        return render_template('dashboard_materiales_historial.html')

    @bp.route('/dashboard/novedades-historial', endpoint='dashboard_novedades_historial')
    def dashboard_novedades_historial():
        return render_template('dashboard_novedades_historial.html')

    @bp.route('/dashboard/sede-estado-manual', endpoint='dashboard_sede_estado_manual')
    def dashboard_sede_estado_manual():
        return render_template('dashboard_sede_estado_manual.html')

    @bp.route('/checklist/interior', methods=['GET', 'POST'])
    def checklist_interior():
        ...
        return render_template('checklist_interior.html', hoy=date.today().isoformat(), registros=registros)
    import sqlite3
    ...
    def sede_aires(codigo):
        ...
        aires = cur.fetchall()
        total = len(aires)
        sin_limpieza = sum((1 for a in aires if not (a['fecha_ultima_limpieza'] or '').strip()))
        fuera_servicio = sum((1 for a in aires if (a['estado'] or '').strip().lower() in ('fuera de servicio', 'no funciona', 'baja')))
        stats = {'total': total, 'sin_limpieza': sin_limpieza, 'fuera_servicio': fuera_servicio}
        return render_template('sede_aires.html', sede=sede, sedes_nav=sedes_nav, aires=aires, stats=stats)

    @bp.route('/sedes/<codigo>/aires/nuevo', methods=['GET', 'POST'])
    ...
    def aire_editar(codigo, aid):
        ...
        if request.method == 'POST':
            ...
            frecuencia_meses = request.form.get('frecuencia_meses') or None
            observaciones = request.form.get('observaciones', '').strip()
            cur.execute('\n                UPDATE aires_mpd\n                   SET ambiente = ?,\n                       marca = ?,\n                       modelo = ?,\n                       tipo = ?,\n                       frigorias = ?,\n                       estado = ?,\n                       fecha_instalacion = ?,\n                       fecha_ultima_limpieza = ?,\n                       frecuencia_meses = ?,\n                       observaciones = ?\n                 WHERE id = ? AND sede_codigo = ?\n            ', (ambiente, marca, modelo, tipo, frigorias, estado, fecha_instalacion, fecha_ultima_limpieza, frecuencia_meses, observaciones, aid, codigo))
            con.commit()
            return redirect(url_for('sede_aires', codigo=codigo))
        return render_template('aire_form.html', sede=sede, aire=aire)

    @bp.route('/sedes/<codigo>/aires/<int:aid>/borrar', methods=['POST'])
    ...
    @bp.route('/sst', methods=['GET', 'POST'], endpoint='sst_general')
    def sst_general():
        con = get_db()
        ...
        con.close()
        return render_template('sst_general.html', sedes=sedes, sst_registros=sst_registros, agentes_sst=agentes_sst, agente_sst_sel=agente_sst_sel, entregas_epp_sst=entregas_epp_sst, incidentes_sst=incidentes_sst, q_agente_id=q_agente_id, q_sede=q_sede, q_tipo=q_tipo, q_estado=q_estado, q_categoria=q_categoria, q_prioridad=q_prioridad, sst_total_personal=sst_total_personal, sst_relevados_personal=sst_relevados_personal, sst_pct_alcance=sst_pct_alcance, sst_pct_riesgo=sst_pct_riesgo, sst_pct_documental=sst_pct_documental, q_buscar=q_buscar)

    @bp.route('/sst/<int:sst_id>/eliminar', methods=['POST'], endpoint='sst_general_eliminar')
    ...
    @bp.route('/sst/<int:sst_id>/editar', methods=['GET', 'POST'], endpoint='sst_general_editar')
    def sst_general_editar(sst_id):
        ...
        if request.method == 'POST':
            ...
            if not fecha or not tipo:
                con.close()
                flash('Fecha y tipo son obligatorios.', 'error')
                return redirect(url_for('sst.sst_general_editar', sst_id=sst_id))
            if sede_codigo == '':
                sede_codigo = None
            con.execute('\n                UPDATE sst_general\n                SET fecha = ?,\n                    sede_codigo = ?,\n                    tipo = ?,\n                    categoria = ?,\n                    area = ?,\n                    titulo = ?,\n                    detalle = ?,\n                    estado = ?,\n                    prioridad = ?,\n                    responsable = ?,\n                    accion_correctiva = ?,\n                    evidencia_url = ?,\n                    fecha_objetivo = ?,\n                    fecha_cierre = ?\n                WHERE id = ?\n            ', (fecha, sede_codigo, tipo, categoria, area, titulo, detalle, estado, prioridad, responsable, accion_correctiva, evidencia_url, fecha_objetivo, fecha_cierre, sst_id))
            con.commit()
            con.close()
            rebuild_eventos_sst()
            ...
            return redirect(url_for('sst.sst_general'))
        con.close()
        return render_template('sst_general_editar.html', sedes=sedes, r=registro)
    ...
    def _seed_sst_control_objetivos(con):
        ...
        if rows and rows['n'] > 0:
            return
        defaults = ['Carteleria', 'Ubicacion de matafuegos', 'Luces de emergencia']
    ...
    def _sst_plan_redirect_next():
        ...
        if return_to == 'cargar':
            return redirect(url_for('sst.sst_plan_cargar'))
        return redirect(url_for('sst.sst_plan'))
    ERGO_DESC_OPTIONS = ['Puesto completo de oficina', 'Silla', 'Escritorio', 'Teclado y raton', 'Monitor', 'Computadora portatil', 'Objetos de uso frecuente', 'Telefono', 'Pausas activas y movilidad']
    ERGO_SILLA_OPTIONS = ['Silla ergonomica', 'Silla fija', 'Silla giratoria']
    ERGO_ESCRITORIO_OPTIONS = ['Escritorio de PC solo', 'Mesa de PC', 'Escritorio profesional en L', 'Escritorio doble (L con dos superficies)']
    ERGO_SOPORTE_OPTIONS = ['Con soporte de monitor', 'Sin soporte de monitor']
    ERGO_ALTURA_MONITOR_OPTIONS = ['Altura correcta (a nivel de ojos)', 'Monitor bajo', 'Monitor alto']
    ERGO_ESPACIO_PIERNAS_OPTIONS = ['Espacio adecuado', 'Espacio reducido', 'Espacio insuficiente']
    ERGO_AJUSTE_ALTURA_OPTIONS = ['No requiere ajuste', 'Subir monitor o escritorio', 'Bajar monitor o escritorio']
    ERGO_NOTEBOOK_OPTIONS = ['No', 'Si, con base o soporte', 'Si, sin base o soporte']
    ERGO_INTERVENCION_OPTIONS = ['Ninguna', 'Ajuste monitor', 'Cambio silla', 'Reubicacion', 'Capacitacion', 'Control administrativo']
    ERGO_SALUD_OPTIONS = [{'label': 'Sin molestias', 'score': 0}, {'label': 'Molestias leves', 'score': 1}, {'label': 'Molestias frecuentes', 'score': 3}, {'label': 'Restriccion medica', 'score': 4}]
    ERGO_SGI_FLOW_STATES = ['Programado', 'Relevado', 'Riesgo evaluado', 'Recomendacion emitida', 'Implementado', 'Verificado', 'Cerrado']
    ERGO_ACCION_OPTIONS = ['Urgente', 'Programado', 'No requiere atencion', 'Cerrado']
    ...
    def _pro_score_parallel(payload):
        ...
        if faltan_clave:
            return {'condicion_0_100': None, 'expo_0_100': None, 'vulner_0_100': None, 'score_final': None, 'condicion_riesgo': 'PENDIENTE', 'motivos': ['Faltan datos para calculo']}
        ...
        score_final = round(0.55 * condicion_0_100 + 0.25 * expo_0_100 + 0.2 * vulner_0_100, 2)
        ...
        if restriccion_medica:
            ...
            nivel = 'RIESGO'
        motivos = _ergo_motivos_riesgo({'altura_monitor': payload.get('altura_monitor'), 'espacio_piernas': payload.get('espacio_piernas'), 'tipo_silla': payload.get('tipo_silla'), 'edad': edad, 'uso_notebook': payload.get('usa_notebook')})
        if horas_pc >= 6:
            motivos.append('Exposicion alta (horas PC)')
        if puntaje_salud >= 3:
            motivos.append('Factor salud')
        return {'condicion_0_100': round(condicion_0_100, 2), 'expo_0_100': round(expo_0_100, 2), 'vulner_0_100': round(vulner_0_100, 2), 'score_final': score_final, 'condicion_riesgo': nivel, 'motivos': motivos[:6]}
    ...
    def build_ergonomia_context(con, sedes):
        ...
        for r in rows:
            d = dict(r)
            dedup_key = ((d.get('codigo_sede') or '').strip().upper(), (d.get('nombre_apellido') or '').strip().upper(), (d.get('correo') or '').strip().lower())
            ...
            d['risk_flags'] = _ergo_risk_flags({'edad': d.get('edad'), 'horas_pc': d.get('horas_pc'), 'puntuacion_puesto': d.get('puntuacion_puesto'), 'accion_tomar': d.get('accion_tomar')})
            d['promedio_pendiente'] = not (_safe_int(d.get('edad'), 0) > 0 and (_safe_int(d.get('puntuacion_salud'), 0) > 0 or (d.get('descripcion_salud') or '').strip()))
        ...
        if data_rows:
            ...
            ergo_selected['risk_flags'] = _ergo_risk_flags({'edad': ergo_selected.get('edad'), 'horas_pc': ergo_selected.get('horas_pc'), 'puntuacion_puesto': ergo_selected.get('puntuacion_puesto'), 'accion_tomar': ergo_selected.get('accion_tomar')})
        promedio_general = round(sum(prom_vals) / len(prom_vals), 2) if prom_vals else 0
        return {'ergo_sede': ergo_sede, 'ergonomia_rows': data_rows, 'ergonomia_total': len(data_rows), 'ergonomia_urgente': counts['urgente'], 'ergonomia_programado': counts['programado'], 'ergonomia_sin_atencion': counts['ok'], 'ergonomia_otros': counts['otro'], 'ergonomia_promedio_general': promedio_general, 'ergonomia_loaded': len(data_rows) > 0, 'ergonomia_error': '', 'ergo_desc_options': ERGO_DESC_OPTIONS, 'ergo_silla_options': ERGO_SILLA_OPTIONS, 'ergo_escritorio_options': ERGO_ESCRITORIO_OPTIONS, 'ergo_soporte_options': ERGO_SOPORTE_OPTIONS, 'ergo_altura_monitor_options': ERGO_ALTURA_MONITOR_OPTIONS, 'ergo_espacio_piernas_options': ERGO_ESPACIO_PIERNAS_OPTIONS, 'ergo_ajuste_altura_options': ERGO_AJUSTE_ALTURA_OPTIONS, 'ergo_notebook_options': ERGO_NOTEBOOK_OPTIONS, 'ergo_intervencion_options': ERGO_INTERVENCION_OPTIONS, 'ergo_salud_options': ERGO_SALUD_OPTIONS, 'ergo_accion_options': ERGO_ACCION_OPTIONS, 'ergo_personal_id': ergo_personal_id, 'ergo_selected': ergo_selected, 'ergo_manual_url': url_for('sst.sst_ergonomia_manual') if 'sst_ergonomia_manual' in app.view_functions else None, 'ergo_gestion_url': url_for('sst.sst_ergonomia_gestion_riesgo') if 'sst_ergonomia_gestion_riesgo' in app.view_functions else None, 'ergo_sedes': [{'codigo': s['codigo'], 'nombre': s['nombre']} for s in sedes]}
    ...
    def build_sst_plan_context(show_carga=False, edit_obj=None, edit_acc=None, sst_view='all'):
        ...
        for o in control_rows:
            ...
            impl_steps = ['impl_ejecucion', 'impl_colocacion'] if compra_nec in ('NO', 'N/A') else ['impl_pedido', 'impl_recibido', 'impl_ejecucion', 'impl_colocacion']
            ...
            def _short_date(val):
                ...
                try:
                    ...
                    return val
            control_objetivos.append({'id': o['id'], 'nombre': o['nombre'], 'done_count': done_count, 'total_count': total_count, 'done_sedes': done_sedes, 'missing_sedes': missing_sedes, 'pct': pct, 'pct_class': pct_class, 'consolidado_ok': consolidado_ok, 'decision_ok': decision_ok, 'decision_ok_effective': decision_ok_effective, 'auto_decision': auto_decision, 'min_sedes_impl': min_sedes_impl, 'impl_progress_pct': impl_progress_pct, 'impl_progress_class': impl_progress_class, 'eval_progress_pct': eval_progress_pct, 'eval_progress_class': eval_progress_class, 'impl_compra_necesaria': o['impl_compra_necesaria'], 'impl_pedido': o['impl_pedido'], 'impl_recibido': o['impl_recibido'], 'impl_ejecucion': o['impl_ejecucion'], 'impl_colocacion': o['impl_colocacion'], 'impl_pedido_fecha': o['impl_pedido_fecha'], 'impl_recibido_fecha': o['impl_recibido_fecha'], 'impl_ejecucion_fecha': o['impl_ejecucion_fecha'], 'impl_colocacion_fecha': o['impl_colocacion_fecha'], 'impl_pedido_fecha_short': _short_date(o['impl_pedido_fecha']), 'impl_recibido_fecha_short': _short_date(o['impl_recibido_fecha']), 'impl_ejecucion_fecha_short': _short_date(o['impl_ejecucion_fecha']), 'impl_colocacion_fecha_short': _short_date(o['impl_colocacion_fecha']), 'eval_verificado': o['eval_verificado'], 'eval_observaciones': o['eval_observaciones'], 'eval_cerrado': o['eval_cerrado'], 'eval_ok_effective': eval_ok_effective})
        ergonomia = build_ergonomia_context(con, sedes)
        con.close()
        return {'sedes': sedes, 'sedes_fueros': sedes_fueros, 'sedes_control': sedes_control, 'objetivos': objetivos, 'control_objetivos': control_objetivos, 'total_obj': len(objetivos), 'total_cumplidos': total_cumplidos, 'total_en_curso': total_en_curso, 'total_riesgo': total_riesgo, 'ergonomia_rows': ergonomia.get('ergonomia_rows', []), 'ergonomia_total': ergonomia.get('ergonomia_total', 0), 'ergonomia_urgente': ergonomia.get('ergonomia_urgente', 0), 'ergonomia_programado': ergonomia.get('ergonomia_programado', 0), 'ergonomia_sin_atencion': ergonomia.get('ergonomia_sin_atencion', 0), 'ergonomia_otros': ergonomia.get('ergonomia_otros', 0), 'ergonomia_promedio_general': ergonomia.get('ergonomia_promedio_general', 0), 'ergonomia_loaded': ergonomia.get('ergonomia_loaded', False), 'ergonomia_error': ergonomia.get('ergonomia_error', ''), 'ergo_desc_options': ergonomia.get('ergo_desc_options', ERGO_DESC_OPTIONS), 'ergo_silla_options': ergonomia.get('ergo_silla_options', ERGO_SILLA_OPTIONS), 'ergo_escritorio_options': ergonomia.get('ergo_escritorio_options', ERGO_ESCRITORIO_OPTIONS), 'ergo_soporte_options': ergonomia.get('ergo_soporte_options', ERGO_SOPORTE_OPTIONS), 'ergo_altura_monitor_options': ergonomia.get('ergo_altura_monitor_options', ERGO_ALTURA_MONITOR_OPTIONS), 'ergo_espacio_piernas_options': ergonomia.get('ergo_espacio_piernas_options', ERGO_ESPACIO_PIERNAS_OPTIONS), 'ergo_ajuste_altura_options': ergonomia.get('ergo_ajuste_altura_options', ERGO_AJUSTE_ALTURA_OPTIONS), 'ergo_notebook_options': ergonomia.get('ergo_notebook_options', ERGO_NOTEBOOK_OPTIONS), 'ergo_intervencion_options': ergonomia.get('ergo_intervencion_options', ERGO_INTERVENCION_OPTIONS), 'ergo_salud_options': ergonomia.get('ergo_salud_options', ERGO_SALUD_OPTIONS), 'ergo_accion_options': ergonomia.get('ergo_accion_options', ERGO_ACCION_OPTIONS), 'ergo_manual_url': ergonomia.get('ergo_manual_url'), 'ergo_gestion_url': ergonomia.get('ergo_gestion_url'), 'ergo_sedes': ergonomia.get('ergo_sedes', []), 'ergo_sede': ergonomia.get('ergo_sede', ''), 'ergo_personal_id': ergonomia.get('ergo_personal_id', 0), 'ergo_selected': ergonomia.get('ergo_selected'), 'sst_view': sst_view, 'show_carga': show_carga, 'edit_obj': edit_obj, 'edit_acc': edit_acc}

    @bp.route('/sst/plan', methods=['GET', 'POST'], endpoint='sst_plan')
    def sst_plan():
        con = get_db()
        con.close()
        if request.method == 'POST':
    ...
    @bp.route('/sst/plan/cargar', methods=['GET'], endpoint='sst_plan_cargar')
    def sst_plan_cargar():
        context = build_sst_plan_context(show_carga=True, sst_view=request.args.get('vista') or 'all')
        return render_template('sst_plan.html', **context)

    @bp.route('/sst/plan/<int:oid>/editar', methods=['GET', 'POST'], endpoint='sst_plan_editar')
    def sst_plan_editar(oid):
        ...
        if request.method == 'POST':
            ...
            if not titulo:
                flash('El titulo es obligatorio.', 'error')
                return redirect(url_for('sst.sst_plan_editar', oid=oid, next=request.args.get('next')))
            try:
                horizonte_val = int(horizonte_meses) if horizonte_meses else None
            except Exception:
                horizonte_val = None
            con = get_db()
            con.execute('\n                UPDATE sst_objetivos\n                SET sede_codigo = ?,\n                    codigo = ?,\n                    titulo = ?,\n                    horizonte_meses = ?,\n                    descripcion = ?,\n                    fecha_inicio = ?,\n                    fecha_fin = ?,\n                    estado = ?,\n                    prioridad = ?\n                WHERE id = ?\n            ', (sede_codigo, codigo, titulo, horizonte_val, descripcion, fecha_inicio, fecha_fin, estado, prioridad, oid))
            con.commit()
            con.close()
    ...
    @bp.route('/sst/plan/<int:oid>/eliminar', methods=['POST'], endpoint='sst_plan_eliminar')
    ...
    @bp.route('/sst/plan/accion/<int:oid>/agregar', methods=['POST'], endpoint='sst_plan_accion_agregar')
    ...
    @bp.route('/sst/plan/accion/<int:aid>/editar', methods=['GET', 'POST'], endpoint='sst_plan_accion_editar')
    def sst_plan_accion_editar(aid):
        ...
        if request.method == 'POST':
            ...
            if not nombre:
                flash('Completa el nombre de la accion.', 'warning')
                return redirect(url_for('sst.sst_plan_accion_editar', aid=aid, next=request.args.get('next')))
            try:
                avance_val = int(avance_pct) if avance_pct != '' else None
            except Exception:
                avance_val = None
            con = get_db()
            con.execute('\n                UPDATE sst_objetivo_acciones\n                SET nombre = ?,\n                    fase = ?,\n                    responsable_area = ?,\n                    estado = ?,\n                    fecha_inicio = ?,\n                    fecha_fin = ?,\n                    avance_pct = ?,\n                    indicador = ?,\n                    clasificacion = ?,\n                    justificacion = ?,\n                    evidencia_url = ?,\n                    notas = ?\n                WHERE id = ?\n            ', (nombre, fase, responsable_area, estado, fecha_inicio, fecha_fin, avance_val, indicador, clasificacion, justificacion, evidencia_url, notas, aid))
            con.commit()
            con.close()
    ...
    @bp.route('/sst/cuadro-unico', methods=['GET'], endpoint='sst_cuadro_unico')
    def sst_cuadro_unico():
        context = build_sst_plan_context(show_carga=False, sst_view='cuadro')
        return render_template('sst_plan.html', **context)

    @bp.route('/sst/ergonomia', methods=['GET'], endpoint='sst_ergonomia_panel')
    def sst_ergonomia_panel():
        context = build_sst_plan_context(show_carga=False, sst_view='ergonomia')
        return render_template('sst_plan.html', **context)

    @bp.route('/sst/plan-gantt', methods=['GET'], endpoint='sst_plan_gantt')
    def sst_plan_gantt():
        context = build_sst_plan_context(show_carga=False, sst_view='gantt')
        return render_template('sst_plan.html', **context)

    @bp.route('/sst/plan/accion/<int:aid>/eliminar', methods=['POST'], endpoint='sst_plan_accion_eliminar')
    ...
    @bp.route('/sst/control/estado/<int:oid>', methods=['POST'])
    def sst_control_estado(oid):
        ...
        con = get_db()
        con.execute('\n            UPDATE sst_control_objetivos\n            SET consolidado_ok = ?,\n                decision_ok = ?,\n                impl_compra_necesaria = ?,\n                impl_pedido = ?,\n                impl_recibido = ?,\n                impl_ejecucion = ?,\n                impl_colocacion = ?,\n                impl_pedido_fecha = ?,\n                impl_recibido_fecha = ?,\n                impl_ejecucion_fecha = ?,\n                impl_colocacion_fecha = ?,\n                eval_verificado = ?,\n                eval_observaciones = ?,\n                eval_cerrado = ?\n            WHERE id = ?\n        ', (consolidado_ok, decision_ok, impl_compra_necesaria or None, impl_pedido or None, impl_recibido or None, impl_ejecucion or None, impl_colocacion or None, impl_pedido_fecha or None, impl_recibido_fecha or None, impl_ejecucion_fecha or None, impl_colocacion_fecha or None, eval_verificado or None, eval_observaciones or None, eval_cerrado or None, oid))
        con.commit()
        con.close()
        flash('Estado actualizado.', 'success')
        return _sst_plan_redirect_next()

    @bp.route('/sst/control/relevamientos/<int:oid>', methods=['POST'])
    ...
    @bp.route('/sst/ergonomia/guardar', methods=['POST'], endpoint='sst_ergonomia_guardar')
    def sst_ergonomia_guardar():
        ...
        audit_payload = {'fecha_relevamiento': fecha_relevamiento, 'evaluador': evaluador, 'horas_pc': horas_pc, 'uso_notebook': uso_notebook, 'accion_tomar': accion_tomar_input, 'responsable': responsable, 'fecha_implementacion': fecha_implementacion, 'fecha_verificacion': fecha_verificacion, 'verificado': verificado, 'observaciones': observaciones, 'intervencion_realizada': intervencion_realizada, 'edad': edad_final, 'puntuacion_puesto': score_puesto, 'altura_monitor': altura_monitor, 'ajuste_altura': ajuste_altura}
        audit_alertas = _ergo_build_audit_alerts(audit_payload)
        estado_flujo = _ergo_next_flow_state(audit_payload)
        pro = _pro_score_parallel({'puntaje_puesto': score_puesto, 'puntaje_sistema': puntaje_sistema, 'puntaje_mobiliario': puntaje_mobiliario, 'puntaje_salud': punt_salud, 'horas_pc': horas_pc, 'edad': edad_final, 'usa_notebook': uso_notebook, 'dolor_reportado': 0, 'restriccion_medica': 0, 'altura_monitor': altura_monitor, 'espacio_piernas': espacio_piernas, 'tipo_silla': tipo_silla})
        ...
        usuario_cambio = (request.headers.get('X-User') or '').strip() or evaluador or 'sistema'
        con.execute("\n            UPDATE sst_ergonomia\n            SET descripcion_puesto = ?,\n                tipo_silla = ?,\n                tipo_escritorio = ?,\n                soporte_monitor = ?,\n                altura_monitor = ?,\n                espacio_piernas = ?,\n                ajuste_altura = ?,\n                horas_pc = ?,\n                uso_notebook = ?,\n                fecha_relevamiento = ?,\n                evaluador = ?,\n                fecha_nacimiento = ?,\n                puntuacion_edad = ?,\n                edad = ?,\n                fecha_implementacion = COALESCE(NULLIF(?, ''), fecha_implementacion),\n                responsable = COALESCE(NULLIF(?, ''), responsable),\n                evidencia_url = COALESCE(NULLIF(?, ''), evidencia_url),\n                fecha_verificacion = COALESCE(NULLIF(?, ''), fecha_verificacion),\n                verificado = CASE WHEN ? IN (0,1) THEN ? ELSE COALESCE(verificado,0) END,\n                intervencion_realizada = ?,\n                fecha_cierre = CASE\n                    WHEN ? IS NOT NULL AND ? != '' THEN ?\n                    ELSE fecha_cierre\n                END,\n                fecha_recordatorio = COALESCE(NULLIF(?, ''), fecha_recordatorio),\n                pro_condicion_0_100 = ?,\n                pro_expo_0_100 = ?,\n                pro_vulner_0_100 = ?,\n                pro_score_final = ?,\n                pro_condicion_riesgo = ?,\n                pro_motivos = ?,\n                estado_flujo = ?,\n                audit_alertas = ?,\n                descripcion_salud = CASE\n                    WHEN ? = '' THEN descripcion_salud\n                    ELSE ?\n                END,\n                puntuacion_puesto = ?,\n                promedio = ?,\n                accion_tomar = ?,\n                observaciones = ?,\n                actualizado_en = datetime('now')\n            WHERE personal_id = ?\n        ", (descripcion_puesto, tipo_silla, tipo_escritorio, soporte_monitor, altura_monitor, espacio_piernas, ajuste_altura, horas_pc, uso_notebook, fecha_relevamiento or None, evaluador or None, fecha_nacimiento or None, punt_edad, edad_final, fecha_implementacion, responsable, evidencia_url, fecha_verificacion, verificado, verificado, intervencion_realizada, fecha_cierre, fecha_cierre, fecha_cierre, fecha_recordatorio, pro.get('condicion_0_100'), pro.get('expo_0_100'), pro.get('vulner_0_100'), pro.get('score_final'), pro.get('condicion_riesgo'), json.dumps(pro.get('motivos', []), ensure_ascii=False), estado_flujo, json.dumps(audit_alertas, ensure_ascii=False), descripcion_salud, descripcion_salud, score_puesto, promedio, accion_tomar_auto, observaciones or None, personal_id))
        ...
        if updated_row:
            ...
            con.execute('\n                INSERT INTO sst_ergonomia_historial (\n                    personal_id,\n                    usuario_cambio,\n                    accion_tomar,\n                    puntuacion_puesto,\n                    promedio,\n                    evaluador,\n                    fecha_relevamiento,\n                    observaciones,\n                    cambios_json,\n                    snapshot_json\n                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\n            ', (personal_id, usuario_cambio, snapshot.get('accion_tomar'), snapshot.get('puntuacion_puesto'), snapshot.get('promedio'), snapshot.get('evaluador'), snapshot.get('fecha_relevamiento'), snapshot.get('observaciones'), json.dumps(changed, ensure_ascii=False), json.dumps(snapshot, ensure_ascii=False)))
        con.commit()
        con.close()
        if is_ajax:
            return jsonify({'ok': True, 'personal_id': personal_id, 'edad': edad_final, 'punt_edad': punt_edad, 'puntuacion_puesto': score_puesto, 'promedio': promedio, 'estado_flujo': estado_flujo, 'audit_alertas': audit_alertas, 'accion_raw': accion_tomar_auto, 'accion_label': _ergo_action_label(accion_tomar_auto), 'risk_flags': _ergo_risk_flags(audit_payload), 'semaforo': _ergo_semaforo(_ergo_total_score(punt_edad, score_puesto, punt_salud)), 'motivos_riesgo': _ergo_motivos_riesgo({'altura_monitor': altura_monitor, 'espacio_piernas': espacio_piernas, 'tipo_silla': tipo_silla, 'edad': edad_final, 'uso_notebook': uso_notebook}), 'dias_desde_eval': _ergo_days_since(fecha_relevamiento), 'pro': pro, 'message': 'Relevamiento ergonomico actualizado.'})
    ...
    @bp.route('/sst/ergonomia/manual', methods=['GET'], endpoint='sst_ergonomia_manual')
    def sst_ergonomia_manual():
        return render_template('sst_ergonomia_manual.html')

    @bp.route('/sst/ergonomia/gestion-riesgo', methods=['GET'], endpoint='sst_ergonomia_gestion_riesgo')
    def sst_ergonomia_gestion_riesgo():
        ...
        pyramid_level = _ergo_recommended_pyramid_level({'intervencion_realizada': intervencion})
        if not accion:
            accion = 'Pendiente'
        volver_url = url_for('sst.sst_plan', ergo_sede=sede or None, ergo_personal_id=personal_id or None)
        if personal_id:
            volver_url = f'{volver_url}#ergo-carga-box'
        return render_template('sst_ergonomia_gestion_riesgo.html', persona=persona, sede=sede, estado_flujo=estado_flujo, estado_ui=estado_ui, paso_actual=paso_actual, accion=accion, intervencion=intervencion, motivos=motivos, pyramid_level=pyramid_level, volver_url=volver_url)

    @bp.route('/sst/ergonomia/salud/guardar', methods=['POST'], endpoint='sst_ergonomia_salud_guardar')
    def sst_ergonomia_salud_guardar():
        ...
        if (d.get('tipo_escritorio') or '') in ('Mesa de PC', 'Escritorio de PC solo'):
            puntaje_mobiliario += 1
        pro = _pro_score_parallel({'puntaje_puesto': punt_puesto, 'puntaje_sistema': puntaje_sistema, 'puntaje_mobiliario': puntaje_mobiliario, 'puntaje_salud': punt_salud, 'horas_pc': _safe_int(d.get('horas_pc'), 0), 'edad': _safe_int(d.get('edad'), 0), 'usa_notebook': d.get('uso_notebook'), 'dolor_reportado': 1 if salud_desc.strip().lower() in ('molestias frecuentes', 'restriccion medica') else 0, 'restriccion_medica': 1 if salud_desc.strip().lower() == 'restriccion medica' else 0, 'altura_monitor': d.get('altura_monitor'), 'espacio_piernas': d.get('espacio_piernas'), 'tipo_silla': d.get('tipo_silla')})
        accion_auto = _accion_from_pro_bucket(pro.get('condicion_riesgo'))
        if (d.get('accion_tomar') or '').strip() == 'Cerrado':
            accion_auto = 'Cerrado'
        payload = {'fecha_relevamiento': d.get('fecha_relevamiento') or '', 'evaluador': d.get('evaluador') or '', 'horas_pc': d.get('horas_pc') or 0, 'uso_notebook': d.get('uso_notebook') or '', 'accion_tomar': accion_auto, 'responsable': d.get('responsable') or '', 'fecha_implementacion': d.get('fecha_implementacion') or '', 'fecha_verificacion': d.get('fecha_verificacion') or '', 'verificado': _safe_int(d.get('verificado'), 0), 'observaciones': d.get('observaciones') or '', 'edad': _safe_int(d.get('edad'), 0), 'puntuacion_puesto': punt_puesto, 'altura_monitor': d.get('altura_monitor') or '', 'ajuste_altura': d.get('ajuste_altura') or ''}
        estado_flujo = _ergo_next_flow_state(payload)
        alertas = _ergo_build_audit_alerts(payload)
        con.execute("\n            UPDATE sst_ergonomia\n            SET descripcion_salud = ?,\n                puntuacion_salud = ?,\n                salud_evaluador = ?,\n                salud_fecha = ?,\n                promedio = ?,\n                accion_tomar = ?,\n                estado_flujo = ?,\n                pro_condicion_0_100 = ?,\n                pro_expo_0_100 = ?,\n                pro_vulner_0_100 = ?,\n                pro_score_final = ?,\n                pro_condicion_riesgo = ?,\n                pro_motivos = ?,\n                audit_alertas = ?,\n                actualizado_en = datetime('now')\n            WHERE personal_id = ?\n        ", (salud_desc or None, punt_salud, salud_eval or None, salud_fecha, promedio, accion_auto, estado_flujo, pro.get('condicion_0_100'), pro.get('expo_0_100'), pro.get('vulner_0_100'), pro.get('score_final'), pro.get('condicion_riesgo'), json.dumps(pro.get('motivos', []), ensure_ascii=False), json.dumps(alertas, ensure_ascii=False), personal_id))
        snap = con.execute('SELECT * FROM sst_ergonomia WHERE personal_id = ?', (personal_id,)).fetchone()
        if snap:
            s = dict(snap)
            con.execute('\n                INSERT INTO sst_ergonomia_historial (\n                    personal_id,\n                    usuario_cambio,\n                    accion_tomar,\n                    puntuacion_puesto,\n                    promedio,\n                    evaluador,\n                    fecha_relevamiento,\n                    observaciones,\n                    cambios_json,\n                    snapshot_json\n                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\n            ', (personal_id, salud_eval or 'dpto_medico', s.get('accion_tomar'), s.get('puntuacion_puesto'), s.get('promedio'), s.get('evaluador'), s.get('fecha_relevamiento'), s.get('observaciones'), json.dumps({'evento': 'Carga salud separada'}, ensure_ascii=False), json.dumps(s, ensure_ascii=False)))
        con.commit()
        con.close()
    ...
    @bp.route('/sst/ergonomia/reevaluar', methods=['POST'], endpoint='sst_ergonomia_reevaluar')
    def sst_ergonomia_reevaluar():
        ...
        if current:
            snap = dict(current)
            con.execute('\n                INSERT INTO sst_ergonomia_historial (\n                    personal_id,\n                    usuario_cambio,\n                    accion_tomar,\n                    puntuacion_puesto,\n                    promedio,\n                    evaluador,\n                    fecha_relevamiento,\n                    observaciones,\n                    cambios_json,\n                    snapshot_json\n                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\n            ', (personal_id, 'sistema', snap.get('accion_tomar'), snap.get('puntuacion_puesto'), snap.get('promedio'), snap.get('evaluador'), snap.get('fecha_relevamiento'), snap.get('observaciones'), json.dumps({'evento': 'Reevaluacion iniciada'}, ensure_ascii=False), json.dumps(snap, ensure_ascii=False)))
    ...
    @bp.route('/sst/ergonomia/cerrar', methods=['POST'], endpoint='sst_ergonomia_cerrar')
    ...
    @bp.route('/sst/ergonomia/reporte', methods=['GET'], endpoint='sst_ergonomia_reporte')
    def sst_ergonomia_reporte():
        ...
        return jsonify({'ok': True, 'total': total, 'urgentes': urgentes, 'programados': programados, 'sin_accion_definida': sin_accion, 'verificados': verificados, 'pendientes_verificacion': pendientes_verificacion, 'pendientes_relevamiento': pendientes_relevamiento, 'registros_con_alertas_auditoria': with_alerts, 'promedio_riesgo_general': round(sum(promedio_vals) / len(promedio_vals), 2) if promedio_vals else 0, 'edad_promedio': round(sum(edad_vals) / len(edad_vals), 1) if edad_vals else 0, 'casos_mayores_60': casos_60, 'casos_mayores_70': casos_70, 'casos_con_notebook': casos_notebook, 'horas_pc_promedio': round(sum(horas_vals) / len(horas_vals), 1) if horas_vals else 0, 'flujo_sgi': flow_counts, 'relevamientos_por_sede': sedes})

    @bp.route('/sst/ergonomia/export.csv', methods=['GET'], endpoint='sst_ergonomia_export_csv')
    def sst_ergonomia_export_csv():
        ...
        writer.writerow(['sede', 'nombre_apellido', 'correo', 'edad', 'punt_edad', 'punt_puesto', 'punt_salud', 'promedio', 'semaforo', 'accion_calculada', 'estado_flujo', 'fecha_relevamiento', 'dias_desde_evaluacion', 'evaluador', 'horas_pc', 'uso_notebook', 'intervencion_realizada', 'motivos_riesgo', 'alertas_auditoria', 'observaciones', 'actualizado_en'])
        for r in ergo.get('ergonomia_rows', []):
            ...
            writer.writerow([r.get('codigo_sede', ''), r.get('nombre_apellido', ''), r.get('correo', ''), r.get('edad', ''), r.get('punt_edad', ''), r.get('punt_puesto', ''), r.get('punt_salud', ''), r.get('promedio', '') if r.get('promedio') is not None else 'Pendiente', r.get('semaforo', ''), r.get('accion_label', r.get('accion', '')), r.get('estado_flujo', ''), r.get('fecha_relevamiento', ''), r.get('dias_desde_eval', ''), r.get('evaluador', ''), r.get('horas_pc', ''), r.get('uso_notebook', ''), r.get('intervencion_realizada', ''), motivos_txt, alertas_txt, r.get('observaciones', ''), r.get('actualizado_en', '')])
        csv_data = output.getvalue()
        output.close()
        sede_tag = (ergo.get('ergo_sede') or 'todas').lower()
        fname = f"ergonomia_{sede_tag}_{date.today().strftime('%Y%m%d')}.csv"
        return Response(csv_data, mimetype='text/csv; charset=utf-8', headers={'Content-Disposition': f'attachment; filename={fname}'})

    @bp.route('/sst/ergonomia/pro_validacion', methods=['GET'], endpoint='sst_ergonomia_pro_validacion')
    def sst_ergonomia_pro_validacion():
        ...
        for r in rows:
            ...
            try:
                ...
                motivos = []
            out.append({'personal_id': d.get('personal_id'), 'sede': d.get('codigo_sede'), 'nombre': d.get('nombre_apellido'), 'actual_accion': d.get('accion_tomar'), 'actual_bucket': actual, 'pro_bucket': pro, 'pro_score_final': d.get('pro_score_final'), 'promedio_actual': d.get('promedio'), 'motivos_pro': motivos, 'fecha_relevamiento': d.get('fecha_relevamiento'), 'actualizado_en': d.get('actualizado_en'), 'coincide': actual == pro})
        total = len(out)
        return jsonify({'ok': True, 'total': total, 'coincidencias': coincidencias, 'diferencias': max(0, total - coincidencias), 'ratio_coincidencia': round(coincidencias / total * 100, 2) if total else 0, 'items': out})
