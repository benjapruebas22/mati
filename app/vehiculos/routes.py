...
def register_vehiculos_control(bp, get_db_connection, ensure_cols, rebuild_eventos_vehiculos):
    ...
    _veh_bootstrap_conn = get_db_connection()
    _ensure_viajes_operativo_cols(_veh_bootstrap_conn)
    ensure_cols(_veh_bootstrap_conn, "vehiculos", [("rendimiento_ref", "REAL")])
    _veh_bootstrap_conn.commit()
    _veh_bootstrap_conn.close()
    # ======= END DDL BOOTSTRAP =======
    ...
    def _current_user_chofer_id(conn):
        ...
        # Eliminamos el full-table scan en Python. Si el nombre no hace match exacto (case-insensitive),
        # normalizamos e intentamos de nuevo en SQL o asumimos que lo tenemos que crear.
        # En la vida real, lo mejor es normalizar desde el inicio en la DB, pero con esto ahorramos iterar
        # los 200 choferes cada request.
        target = full_name.lower().strip()
        row = conn.execute(
            "SELECT id FROM agentes_intendencia WHERE lower(agente) = ?",
            (target,),
        ).fetchone()
    ...
    def viaje_editar2(viaje_id):
        return redirect(url_for("vehiculos.viaje_editar", viaje_id=viaje_id))
    ...
    def viaje_eliminar(viaje_id):
        ...
        return redirect(url_for("vehiculos.vehiculos_control_diario", fecha=fecha, desde=desde, hasta=hasta))
    ...
    def viaje_cerrar(viaje_id):
        ...
        if request.method == "POST":
            ...
            if km_fin <= 0:
                ...
                return redirect(url_for("vehiculos.viaje_cerrar", viaje_id=viaje_id))
            ...
            return redirect(url_for("vehiculos.vehiculos_control_diario", fecha=viaje["fecha"]))

        conn.close()
        flash("Cierre manual deshabilitado: usa Editar y completa KM final.", "info")
        return redirect(url_for("vehiculos.viaje_editar", viaje_id=viaje_id, fecha=viaje["fecha"]))
    ...
    def vehiculos_control_diario():
        ...
        if request.method == "POST":
            ...
            if km_ini is None or km_fin is None:
                ...
                conn.close()
                return redirect(url_for("vehiculos.vehiculos_control_diario", fecha=fecha))
    ...
    @bp.route("/vehiculos/nuevo", methods=["GET", "POST"], endpoint="vehiculos_nuevo")
    def vehiculos_nuevo():
        conn = get_db_connection()
    ...
    @bp.route("/vehiculos/documentacion", methods=["GET", "POST"], endpoint="vehiculos_documentacion")
    def vehiculos_documentacion():
        ...
        if request.method == "POST":
            ...
            if tipo_form == "vehiculo_estado":
                ...
                if not patente:
                    ...
                    conn.close()
                    return redirect(url_for("vehiculos.vehiculos_documentacion"))
        ...
        chofer_docs_raw = conn.execute(
            """
            SELECT ai.id, ai.agente, doc.fecha_vencimiento
            FROM agentes_intendencia ai
            LEFT JOIN (
                SELECT agente_id, MAX(fecha_vencimiento) AS fecha_vencimiento
                FROM agentes_documentacion
                WHERE tipo = 'carnet_conducir'
                GROUP BY agente_id
            ) doc ON doc.agente_id = ai.id
    ...
    @bp.route("/vehiculos/estadisticas", methods=["GET"], endpoint="vehiculos_estadisticas")
    ...
    @bp.route("/viajes/<int:viaje_id>/set_km_ini", methods=["POST"], endpoint="viaje_set_km_ini")
    def viaje_set_km_ini(viaje_id):
        ...
        if nuevo < 0:
            ...
            return redirect(url_for("vehiculos.vehiculos_control_diario", fecha=row["fecha"]))
    ...
    # -------------------------
    @bp.route("/vehiculos/combustible", methods=["GET", "POST"], endpoint="vehiculos_combustible")
    def vehiculos_combustible():
        ...
        if request.method == "POST":
            ...
            if not patente:
                ...
                conn.close()
                return redirect(url_for("vehiculos.vehiculos_combustible"))
    ...
    @bp.route("/vehiculos/combustible/estadisticas", methods=["GET"], endpoint="vehiculos_combustible_estadisticas")
    ...
    # -------------------------
    @bp.route("/vehiculos/combustible/nuevo", methods=["GET", "POST"], endpoint="vehiculos_combustible_nuevo")
    ...
    # =========================
    @bp.route("/vehiculos/combustible/remitos/<path:filename>", endpoint="combustible_remito_ver")
    ...
    @bp.route("/viajes/<int:viaje_id>/editar", methods=["GET", "POST"], endpoint="viaje_editar_legacy")
    ...
    @bp.route("/viajes/<int:viaje_id>/eliminar", methods=["POST"], endpoint="viajes_delete")
    ...
    @bp.route("/vehiculos/checklist", methods=["GET", "POST"], endpoint="vehiculos_checklist")
    def vehiculos_checklist():
        ...
        if request.method == "POST":
            ...
            return redirect(url_for("vehiculos.vehiculos_checklist"))
    ...
    @bp.route("/vehiculos/combustible/<int:cid>/editar", methods=["GET", "POST"], endpoint="combustible_editar")
    def combustible_editar(cid):
        ...
        if request.method == "POST":
            ...
            if km_actual < 0 or litros <= 0 or precio_unit <= 0 or importe_real < 0:
                ...
                return redirect(url_for("vehiculos.combustible_editar", cid=cid))
    ...
    @bp.route("/vehiculos/combustible/<int:cid>/eliminar", methods=["POST"], endpoint="combustible_eliminar")
    ...
    # =========================================================
    @bp.route("/vehiculos/basedatos", endpoint="vehiculos_bd_home")
    ...
    # DESTINOS CRUD
    # =========================================================
    @bp.route("/vehiculos/basedatos/destinos", methods=["GET", "POST"], endpoint="bd_destinos")
    def bd_destinos():
        con = get_db()
    ...
    @bp.route("/vehiculos/basedatos/destinos/<int:did>/delete", methods=["POST"], endpoint="bd_destinos_delete")
    def bd_destinos_delete(did):
        ...
        return redirect(url_for("vehiculos.bd_destinos"))


    # =========================================================
    # EQUIPO INTERDISCIPLINARIO CRUD
    # =========================================================
    @bp.route("/vehiculos/basedatos/equipo", methods=["GET", "POST"], endpoint="bd_equipo")
    def bd_equipo():
        con = get_db()
    ...
    @bp.route("/vehiculos/basedatos/equipo/<int:eid>/delete", methods=["POST"], endpoint="bd_equipo_delete")
    def bd_equipo_delete(eid):
        ...
        return redirect(url_for("vehiculos.bd_equipo"))
    ...
    # =========================================================
    @bp.route("/vehiculos/basedatos/choferes", methods=["GET", "POST"], endpoint="bd_choferes")
    def bd_choferes():
        con = get_db()
        ...
        if request.method == "POST":
            ...
            if not agente:
                flash("Nombre obligatorio.", "warning")
                con.close()
                return redirect(url_for("vehiculos.bd_choferes"))
    ...
    @bp.route("/vehiculos/basedatos/choferes/<int:cid>/editar", methods=["GET", "POST"], endpoint="bd_choferes_editar")
    def bd_choferes_editar(cid):
        ...
        if request.method == "POST":
            ...
            if not agente:
                con.close()
                flash("Nombre obligatorio.", "warning")
                return redirect(url_for("vehiculos.bd_choferes_editar", cid=cid))
    ...
    @bp.route("/vehiculos/basedatos/choferes/<int:cid>/baja", methods=["POST"], endpoint="bd_choferes_baja")
    ...
    # REACTIVAR (activo=1)
    @bp.route("/vehiculos/basedatos/choferes/<int:cid>/alta", methods=["POST"], endpoint="bd_choferes_alta")
    ...
    # =========================================================
    @bp.route("/vehiculos/basedatos/choferes-aut", methods=["GET", "POST"], endpoint="bd_choferes_aut")
    def bd_choferes_aut():
        con = get_db()
    ...
    @bp.route("/vehiculos/basedatos/choferes-aut/<int:cid>/delete", methods=["POST"], endpoint="bd_choferes_aut_delete")
    def bd_choferes_aut_delete(cid):
        ...
        return redirect(url_for("vehiculos.bd_choferes_aut"))


    # =========================================================
    # PRECIOS COMBUSTIBLE
    # =========================================================
    @bp.route("/vehiculos/basedatos/precios", methods=["GET", "POST"], endpoint="bd_precios")
    def bd_precios():
        con = get_db()
    ...
    @bp.route("/vehiculos/eventos/regenerar", endpoint="vehiculos_eventos_regenerar")
