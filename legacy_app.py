...
def module_from_path(path: str) -> str:
    ...
    if path.startswith("/api/dashboard"):
        return "dashboard"
    if path.startswith("/asignaciones-simple"):
        return "dashboard"
...
register_asignaciones_simple(app, get_db)
...
def normalizar_local_clave(raw: str) -> str:
    txt = str(raw or "").strip().upper()
    if not txt:
        return ""
    m = re.search(r"\bD\s*0*(\d{1,3})\b", txt)
    if not m:
        return ""
    return f"D{int(m.group(1)):02d}"

def infra_extract_criterio_codigo(raw) -> str:
    txt = str(raw or "").strip().upper()
    if not txt:
        return ""
    m = re.search(r"\bC\s*0*(\d{1,2})\b", txt)
    if not m:
        return ""
...
def infra_sugerir_criterio(
    ...
    kw_mesa_entrada = ("mesa de entrada",)
    kw_audiencia = ("audiencia",)
    kw_no_aplica = ("bano", "cocina", "patio", "pasillo", "archivo", "deposito")
    kw_servicios = ("servicios", "limpieza", "mantenimiento", "intendencia")
    kw_direccion = ("defensora general", "defensoria general", "administrador", "secretario general", "jefe", "coordinador")
    kw_defensor = ("defensor", "defensoria")
    kw_secretario = ("secretario",)
    kw_secretaria_coordinacion = ("secretaria coordinacion", "secretaria de coordinacion")
    kw_defensoria_penal = ("defensoria penal",)
    kw_social = ("civil", "ajs", "juridico social")
    kw_menores = ("menores", "incapaces")
    kw_especial = ("hibrido", "interdisciplinario", "especial", "mixto raro")

    if infra_has_any(full, kw_mesa_entrada):
        return {"codigo": "C07", "motivo": "Mesa de entrada"}
    if infra_has_any(full, kw_audiencia):
        return {"codigo": "C08", "motivo": "Sala de audiencia"}
    if personas == 0 and infra_has_any(full, kw_no_aplica):
        return {"codigo": "C10", "motivo": "Espacio sin analisis operativo"}
    if infra_has_any(full, kw_servicios):
        return {"codigo": "C09", "motivo": "Servicios / apoyo"}
    if infra_has_any(full, kw_secretaria_coordinacion):
        return {"codigo": "C02", "motivo": "Secretaria Coordinacion"}
    if infra_has_any(full, kw_direccion):
        return {"codigo": "C03", "motivo": "Alta direccion / jefatura"}

    if infra_has_any(full, kw_menores):
        if 3 <= personas <= 5:
            return {"codigo": "C04", "motivo": "Menores/Incapaces en equipo"}
        if personas >= 6:
            return {"codigo": "C06", "motivo": "Menores/Incapaces sala auxiliares"}

    if infra_has_any(full, kw_defensoria_penal) and 3 <= personas <= 5:
        return {"codigo": "C04", "motivo": "Defensoria penal de equipo"}

    if infra_has_any(full, kw_social):
        return {"codigo": "C05", "motivo": "Civil / AJS / Juridico Social"}

    if infra_has_any(full, kw_especial):
        return {"codigo": "C00", "motivo": "Caso especial no tipico"}

    has_defensor = infra_has_any(full, kw_defensor)
    has_secretario = infra_has_any(full, kw_secretario)
    if has_defensor and personas == 1:
        return {"codigo": "C01", "motivo": "Defensor individual"}
    if has_defensor and (has_secretario or personas == 2):
        return {"codigo": "C02", "motivo": "Defensor con secretario"}

    if personas == 0:
        return {"codigo": "C10", "motivo": "Sin personal activo"}
    if personas == 1:
        return {"codigo": "C01", "motivo": "Oficina individual generica"}
    if personas == 2:
        return {"codigo": "C11", "motivo": "Oficina comun 2 personas"}
    if personas == 3:
        return {"codigo": "C12", "motivo": "Oficina comun 3 personas"}
    if personas == 4:
        return {"codigo": "C13", "motivo": "Oficina comun 4 personas"}
    if personas >= 5:
        return {"codigo": "C14", "motivo": "Oficina comun 5+ personas"}
    return {"codigo": "C00", "motivo": "Sin patron concluyente: revisar criterio"}
...
def infra_fmt_esperado_simple(rdef):
    if not rdef:
        return "-"
    min_m = str(rdef.get("min_modo") or "FIJO").strip().upper()
    max_m = str(rdef.get("max_modo") or "FIJO").strip().upper()
    min_v = _infra_to_float(rdef.get("min_val"))
    max_v = _infra_to_float(rdef.get("max_val"))

    if min_m == "PERSONAS" and max_m == "PERSONAS":
        return "personas reales"
    if min_m == "PERSONAS" and max_m == "PERSONAS_MAS":
        plus = _infra_to_float(max_v) or 0
        if abs(float(plus)) < 0.0001:
            return "personas reales"
        return f"personas reales + {infra_fmt_num(plus)}"
    if min_m == "PERSONAS_MAS" and max_m == "PERSONAS_MAS":
        return f"personas + {infra_fmt_num(min_v or 0)} a personas + {infra_fmt_num(max_v or 0)}"

    a = infra_fmt_num(min_v)
    b = infra_fmt_num(max_v)
    return a if a == b else f"{a} a {b}"

def infra_parse_esperado_simple(raw):
    txt_raw = str(raw or "").strip()
    txt = infra_norm_text(txt_raw)
    if not txt or txt in ("-", "sin analisis", "no aplica", "editable"):
        return ("FIJO", 0, "FIJO", 0)

    if "personas" in txt:
        m_plus = re.search(r"personas?(?:\s+reales?)?\s*\+\s*([0-9]+(?:[.,][0-9]+)?)", txt)
        if m_plus:
            plus = _infra_to_float(m_plus.group(1)) or 0
            return ("PERSONAS", 0, "PERSONAS_MAS", plus)
        return ("PERSONAS", 0, "PERSONAS", 0)

    nums = re.findall(r"-?\d+(?:[.,]\d+)?", txt_raw)
    vals = [_infra_to_float(n) for n in nums if _infra_to_float(n) is not None]
    if not vals:
        return ("FIJO", 0, "FIJO", 0)
    if len(vals) == 1:
        v = float(vals[0])
        return ("FIJO", v, "FIJO", v)

    a = float(vals[0])
    b = float(vals[1])
    if a > b:
        a, b = b, a
    return ("FIJO", a, "FIJO", b)
...
def infra_accion_sugerida(estado_general: str):
    ...
    if st == "No aplica":
        return "Sin analisis operativo"
    if st == "Especial":
        return "Validar configuracion especial"
    if st == "Sin criterio":
        return "Asignar criterio"
    if st == "Bajo estandar":
        return "Pedido justificado / Buscar stock disponible"
    if st == "Alto":
        return "Revisar redistribucion"
    if st == "Razonable":
        return "Sin prioridad"
    if st == "Revisar inventario":
        return "Completar inventario operativo"
    if st == "Revisar criterio":
        return "Validar criterio aplicado"
    if st == "Revisar":
...
def ensure_infra_criterios_schema(db=None):
    ...
    criterios_seed = [
        ...
        },
        {
            ...
            "fuero": "GENERAL",
            "personas_base": "1",
            "composicion": "Defensora general, administrador, secretario general, jefaturas",
        ...
        },
        {
            ...
            "fuero": "PENAL",
            "personas_base": "3 a 5",
            "composicion": "Defensor, secretario, prosecretario y auxiliares",
            "estado_esperado": "Equipo penal con puestos compartidos",
            "orden": 4,
            "rubros": {
                "escritorio_prof": ("FIJO", 2, "FIJO", 2),
                "mesa_pc": ("FIJO", 2, "FIJO", 4),
                "silla_giratoria": ("PERSONAS", 0, "PERSONAS", 0),
                "silla_fija": ("FIJO", 2, "FIJO", 4),
                "armario_biblioteca": ("FIJO", 1, "FIJO", 2),
                "aire": ("FIJO", 1, "FIJO", 2),
                "puestos_existentes": ("PERSONAS", 0, "PERSONAS", 0),
            },
        },
        {
            "codigo": "C05",
            "nombre": "Defensor + Secretario + Auxiliares",
            "fuero": "JURIDICO SOCIAL",
            "personas_base": "3 a 5",
            "composicion": "Juridico Social / Civil / oficina mixta",
            "estado_esperado": "Oficina mixta flexible",
            "orden": 5,
            "rubros": {
                "escritorio_prof": ("FIJO", 2, "FIJO", 3),
                "mesa_pc": ("FIJO", 3, "FIJO", 5),
                "silla_giratoria": ("PERSONAS", 0, "PERSONAS", 0),
        ...
        },
        {
            ...
            "composicion": "Sala operativa con varios auxiliares",
            "estado_esperado": "Sala operativa amplia",
            "orden": 6,
            "rubros": {
                "escritorio_prof": ("FIJO", 0, "FIJO", 0),
                "mesa_pc": ("PERSONAS", 0, "PERSONAS", 0),
                "silla_giratoria": ("PERSONAS", 0, "PERSONAS", 0),
                "silla_fija": ("FIJO", 2, "FIJO", 4),
        ...
        },
        {
            ...
            "fuero": "GENERAL",
            "personas_base": "0 a 2",
            "composicion": "Limpieza, mantenimiento, intendencia, apoyo",
            "estado_esperado": "Apoyo operativo",
            "orden": 9,
            "rubros": {
                "escritorio_prof": ("FIJO", 0, "FIJO", 1),
                "mesa_pc": ("FIJO", 0, "FIJO", 1),
                "silla_giratoria": ("FIJO", 0, "FIJO", 1),
                "silla_fija": ("FIJO", 0, "FIJO", 4),
        ...
        },
        {
            ...
            "composicion": "Bano, cocina, patio, pasillo, archivo o deposito sin puesto",
        ...
        {
            "codigo": "C11",
            "nombre": "Oficina 2 personas",
            "fuero": "GENERAL",
            "personas_base": "2",
            "composicion": "Oficina comun de 2 personas",
            "estado_esperado": "Oficina generica 2 personas",
            "orden": 11,
            "rubros": {
                "escritorio_prof": ("FIJO", 0, "FIJO", 1),
                "mesa_pc": ("FIJO", 2, "FIJO", 2),
        ...
        {
            "codigo": "C12",
            "nombre": "Oficina 3 personas",
            "fuero": "GENERAL",
            "personas_base": "3",
            "composicion": "Oficina comun de 3 personas",
            "estado_esperado": "Oficina generica 3 personas",
            "orden": 12,
            "rubros": {
                "escritorio_prof": ("FIJO", 0, "FIJO", 1),
                "mesa_pc": ("FIJO", 3, "FIJO", 3),
                "silla_giratoria": ("FIJO", 3, "FIJO", 3),
                "silla_fija": ("FIJO", 2, "FIJO", 4),
                "armario_biblioteca": ("FIJO", 1, "FIJO", 2),
                "aire": ("FIJO", 0, "FIJO", 1),
                "puestos_existentes": ("FIJO", 3, "FIJO", 3),
            },
        },
        {
            "codigo": "C13",
            "nombre": "Oficina 4 personas",
            "fuero": "GENERAL",
            "personas_base": "4",
            "composicion": "Oficina comun de 4 personas",
            "estado_esperado": "Oficina generica 4 personas",
            "orden": 13,
            "rubros": {
                "escritorio_prof": ("FIJO", 0, "FIJO", 1),
                "mesa_pc": ("FIJO", 3, "FIJO", 4),
                "silla_giratoria": ("FIJO", 4, "FIJO", 4),
                "silla_fija": ("FIJO", 2, "FIJO", 4),
        ...
        {
            "codigo": "C14",
            "nombre": "Oficina 5+ personas",
            "fuero": "GENERAL",
            "personas_base": "5 o mas",
            "composicion": "Oficina comun amplia",
            "estado_esperado": "Oficina generica 5+",
            "orden": 14,
            "rubros": {
                "escritorio_prof": ("FIJO", 0, "FIJO", 1),
                "mesa_pc": ("PERSONAS", 0, "PERSONAS", 0),
                "silla_giratoria": ("PERSONAS", 0, "PERSONAS", 0),
                "silla_fija": ("FIJO", 2, "FIJO", 5),
        ...
        {
            "codigo": "C00",
            "nombre": "Deposito especial",
            "fuero": "GENERAL",
            "personas_base": "Variable",
            "composicion": "Comodin editable para casos especiales o hibridos",
            "estado_esperado": "Configuracion manual",
            "orden": 99,
            "rubros": {
                "escritorio_prof": ("FIJO", 0, "FIJO", 0),
                "mesa_pc": ("FIJO", 0, "FIJO", 0),
    ...
    seed_codes = {str(c.get("codigo") or "").strip().upper() for c in criterios_seed}
    aplicar_migracion_base_v2 = any(cd and cd not in existing_codes for cd in seed_codes)
...
def sede_ficha(codigo):
    ...
    LOCALES_POR_PISO_RANGOS = {
        ...
        # Palpalá Civil (S20): PB 01-10 / P1 11-17
    ...
    luminarias_history_rows = []
    luminarias_history_filters = {"fecha": "", "codigo": "", "ambiente": "", "movimiento": ""}
    luminarias_view = "operativo"
    ...
    if tab == "luminarias":
        ...
        luminarias_view = (request.args.get("lview") or "operativo").strip().lower()
        if luminarias_view not in ("operativo", "historial"):
            luminarias_view = "operativo"
        luminarias_history_filters = {
            "fecha": (request.args.get("lh_fecha") or "").strip(),
            "codigo": (request.args.get("lh_codigo") or "").strip(),
            "ambiente": _mobiliario_plano_normalize_local(request.args.get("lh_ambiente") or "") if (request.args.get("lh_ambiente") or "").strip() else "",
            "movimiento": (request.args.get("lh_mov") or "").strip().lower(),
        }
        where = ["l.codigo_sede = ?"]
        params = [codigo]
        ...
        luminarias_history_rows = _luminarias_history_feed(
            ...
            piso,
            local_filter=local,
            fecha_filtro=luminarias_history_filters["fecha"],
            codigo_filtro=luminarias_history_filters["codigo"],
            ambiente_filtro=luminarias_history_filters["ambiente"],
            movimiento_filtro=luminarias_history_filters["movimiento"],
        )
        try:
    ...
    return render_template(
        ...
        luminarias_history_rows=luminarias_history_rows,
        luminarias_history_filters=luminarias_history_filters,
        luminarias_view=luminarias_view,
...
def sedes_resumen_mpd():
    ...
    if request.method == "POST":
        ...
        if action == "save_criterio":
            ...
            for rb in INFRA_RUBROS_META:
                k = rb["key"]
                esperado_txt = request.form.get(f"{k}_esperado")
                if esperado_txt is not None and str(esperado_txt).strip() != "":
                    min_modo, min_val, max_modo, max_val = infra_parse_esperado_simple(esperado_txt)
                else:
                    # Compatibilidad con la grilla tecnica anterior.
                ...
                crit_raw = request.form.get(f"{k}_critico")
                es_critico = int(rb.get("critico") or 0) if crit_raw is None else (1 if crit_raw == "1" else 0)
                db.execute("""
                    INSERT INTO infra_criterios_rubros
                    (criterio_codigo, rubro, min_modo, min_val, max_modo, max_val, es_critico, actualizado_en)
                    VALUES (?,?,?,?,?,?,?, datetime('now'))
        ...
        if action == "assign_criterio":
            ...
            deposito = normalizar_local_clave(request.form.get("asig_deposito"))
    ...
    criterios_opts = sorted(
        criterios_map.values(),
        key=lambda c: (int(c.get("orden") or 0), str(c.get("codigo") or "")),
    )
    ...
    for rb in INFRA_RUBROS_META:
        rk = rb["key"]
        rv = criterio_edit["rubros"].setdefault(
            rk,
            {"min_modo": "FIJO", "min_val": 0, "max_modo": "FIJO", "max_val": 0, "es_critico": int(rb["critico"] or 0)},
        )
        rv["esperado_txt"] = infra_fmt_esperado_simple(rv)
    ...
    per_cols = {str(c["name"] or "").strip().lower() for c in db.execute("PRAGMA table_info(personal_sede)").fetchall()}
    dep_expr = "COALESCE(dependencia,'') AS dependencia" if "dependencia" in per_cols else "'' AS dependencia"
    piso_expr = "COALESCE(piso,'') AS piso" if "piso" in per_cols else "'' AS piso"
    mail_expr = "COALESCE(email_admin,'') AS email_admin" if "email_admin" in per_cols else "'' AS email_admin"
    criterio_col = ""
    for cc in ("criterio_codigo", "criterio", "criterio_estandar"):
        if cc in per_cols:
            criterio_col = cc
            break
    ...
    mob_rows = db.execute("""
        SELECT
            UPPER(COALESCE(codigo_sede,'')) AS sede_codigo,
            COALESCE(codigo_local,'') AS codigo_local,
            COALESCE(MAX(COALESCE(descripcion,'')),'') AS descripcion,
            COALESCE(SUM(COALESCE(aire_marca,0)),0) AS aire_marca,
            COALESCE(SUM(COALESCE(escritorio_prof,0)),0) AS escritorio_prof,
            COALESCE(SUM(COALESCE(mesa_pc,0)),0) AS mesa_pc,
            COALESCE(SUM(COALESCE(silla_giratoria,0)),0) AS silla_giratoria,
            ...
            COALESCE(SUM(COALESCE(biblioteca_baja,0)),0) AS biblioteca_baja,
            COALESCE(SUM(COALESCE(otros,0)),0) AS otros,
            COALESCE(MAX(COALESCE(otros_detalle,'')),'') AS observaciones
        FROM mobiliario_sede
        WHERE COALESCE(activo,1)=1
    ...
    for m in mob_rows:
        ...
        dep = normalizar_local_clave(m["codigo_local"])
        if not sede or not dep:
            continue
        k = (sede, dep)
        inventario_keys.add(k)
        mobiliario_map[k]["descripcion"] = str(m["descripcion"] or "").strip()
        mobiliario_map[k]["aire"] = float(m["aire_marca"] or 0)
        ...
        mobiliario_map[k]["otros"] = float(m["otros"] or 0)
        mobiliario_map[k]["observaciones"] = str(m["observaciones"] or "").strip()
    ...
    for pr in pto_rows:
        ...
        dep = normalizar_local_clave(pr["codigo_local"])
        if not sede or not dep:
            continue
    ...
    for sede, dep in sorted(all_keys, key=lambda x: (x[0], x[1])):
        ...
        descripcion_inv = str(inv_info.get("descripcion") or "").strip()
        referencia = descripcion_inv or str(info.get("referencia") or "").strip() or dep
        personas_reales = int(personal_map.get((sede, dep), {}).get("personas_reales", 0) or 0)
        nombres = personal_map.get((sede, dep), {}).get("nombres", [])
        pisos_counter = personal_map.get((sede, dep), {}).get("pisos", {})
        piso_detectado = "-"
        if pisos_counter:
            piso_detectado = sorted(pisos_counter.items(), key=lambda x: (-int(x[1]), str(x[0])))[0][0]
        emails_set = sorted(list(personal_map.get((sede, dep), {}).get("emails", set())))
        email_ref = emails_set[0] if emails_set else ""
        inventario_presente = (sede, dep) in inventario_keys
        ...
        criterios_personal_counter = personal_map.get((sede, dep), {}).get("criterios", {})
        criterio_personal_codigo = ""
        if criterios_personal_counter:
            criterio_personal_codigo = sorted(
                criterios_personal_counter.items(),
                key=lambda x: (-int(x[1]), str(x[0])),
            )[0][0]
        criterio_personal = criterios_map.get(criterio_personal_codigo)
        ...
        dependencia_detectada = infra_detect_dependencia(
            ...
            referencia=("sin personal" if personas_reales == 0 else referencia),
        ...
        elif criterio_personal:
            criterio_final = criterio_personal
            criterio_final_codigo = criterio_personal_codigo
            criterio_final_fuente = "Personal"
        else:
            criterio_final = criterio_sugerido
            criterio_final_codigo = criterio_sugerido_codigo
            criterio_final_fuente = "Sugerido"

        criterio_final_no_aplica = (
            criterio_final_codigo == "C10"
            or (criterio_final_codigo == "C09" and personas_reales <= 0)
        )
        criterio_final_revisar_forzado = (criterio_final_codigo == "C00" and criterio_final_fuente == "Sugerido")
        criterio_final_especial = (criterio_final_codigo == "C00" and criterio_final_fuente in ("Manual", "Personal"))
        criterio_final_operativo = bool(criterio_final) and (not criterio_final_no_aplica) and (not criterio_final_revisar_forzado) and (not criterio_final_especial)
        criterio_final_nombre = (
            str(criterio_final.get("nombre") or "").strip()
            if criterio_final
            else ("Sin analisis" if criterio_final_codigo == "C10" else ("Deposito especial" if criterio_final_codigo == "C00" else "Sin criterio"))
        )
        criterio_sugerido_nombre = (
            str(criterio_sugerido.get("nombre") or "").strip()
            if criterio_sugerido
            else ("Sin analisis" if criterio_sugerido_codigo == "C10" else ("Deposito especial" if criterio_sugerido_codigo == "C00" else "Sin criterio"))
        )
        ...
        reales = {
            ...
            "aire": float(mobiliario_map.get((sede, dep), {}).get("aire", 0) or 0),
        ...
        for rb in INFRA_RUBROS_META:
            ...
            max_esp = None
            esperado_txt = "-"
            if criterio_final_no_aplica:
                esperado_txt = "No aplica"
                estado = "No aplica"
            ...
            if criterio_final_especial:
                rdef = criterio_final["rubros"].get(rk) if criterio_final else None
                esperado_txt = infra_fmt_esperado_simple(rdef) if rdef else "Editable"
                estado = "Especial"
                rubros_estado[rk] = estado
                rubros_min[rk] = min_esp
                rubros_data[rk] = {
            ...
            if criterio_final_revisar_forzado:
                esperado_txt = "Editable"
                estado = "Revisar criterio"
                rubros_estado[rk] = estado
                rubros_min[rk] = min_esp
                rubros_data[rk] = {
                    "real": real_val,
        ...
        if personas_reales > 0 and not inventario_presente:
            estado_general = "Revisar inventario"
        elif criterio_final_no_aplica:
            estado_general = "No aplica"
        elif criterio_final_especial:
            estado_general = "Especial"
        elif criterio_final_revisar_forzado:
            estado_general = "Revisar criterio"
        elif not criterio_final_operativo:
            estado_general = "Sin criterio" if criterio_final_codigo in ("",) else "Revisar"
        else:
        ...
        row = {
            ...
            "local": dep,
            "piso": piso_detectado,
            "descripcion_inventario": descripcion_inv or nombre_ref or "-",
            ...
            "criterio_personal_codigo": criterio_personal_codigo or "",
            "criterio_personal_nombre": (str(criterio_personal.get("nombre") or "").strip() if criterio_personal else ""),
            ...
            "inventario_otros": float(inv_info.get("otros", 0) or 0),
            "inventario_observaciones": str(inv_info.get("observaciones") or "").strip(),
            "rubros": rubros_data,
    ...
    for sede, dep in all_keys:
        dep_filter_map.setdefault(dep, {"label": dep, "sedes": set()})
        dep_filter_map[dep]["sedes"].add(sede)
        dsc = str(mobiliario_map.get((sede, dep), {}).get("descripcion") or "").strip()
        if not dsc:
            dsc = str(dep_catalog.get((sede, dep), {}).get("descripcion") or "").strip()
        if dsc and dep_filter_map[dep]["label"] == dep:
            dep_filter_map[dep]["label"] = f"{dep} - {dsc}"
    ...
    for sede in sorted(resumen_sede_map.keys()):
        ...
        elif item["especial"] > 0 and item["bajo_estandar"] == 0 and item["alto"] == 0:
            conclusion = "Existen depositos especiales: validar configuracion manual y seguimiento."
        elif item["bajo_estandar"] == 0 and item["alto"] == 0 and item["revisar"] == 0 and item["razonable"] == 0 and item["no_aplica"] > 0:
            conclusion = "Predominan espacios sin analisis operativo."
    ...
    def _fmt_ref_range(rdef):
        if not rdef:
            return "-"
        min_m = str(rdef.get("min_modo") or "FIJO").upper()
        max_m = str(rdef.get("max_modo") or "FIJO").upper()
        min_v = _infra_to_float(rdef.get("min_val"))
        max_v = _infra_to_float(rdef.get("max_val"))
        if min_m == "PERSONAS" and max_m == "PERSONAS":
            return "personas reales"
        if min_m == "PERSONAS" and max_m == "PERSONAS_MAS":
            plus = infra_fmt_num(max_v or 0)
            return "personas reales" if plus == "0" else f"personas reales a personas + {plus}"
        if min_m == "FIJO" and max_m == "FIJO":
            a = infra_fmt_num(min_v)
            b = infra_fmt_num(max_v)
            return a if a == b else f"{a} a {b}"
        return f"{infra_fmt_num(min_v)} a {infra_fmt_num(max_v)}"

    infra_criterios_ref = []
    for c in criterios_opts:
        rb = c.get("rubros") or {}
        infra_criterios_ref.append({
            "codigo": c.get("codigo"),
            "nombre": c.get("nombre"),
            "esc_prof": _fmt_ref_range(rb.get("escritorio_prof")),
            "mesa_pc": _fmt_ref_range(rb.get("mesa_pc")),
            "giratoria": _fmt_ref_range(rb.get("silla_giratoria")),
            "fija": _fmt_ref_range(rb.get("silla_fija")),
            "armario": _fmt_ref_range(rb.get("armario_biblioteca")),
            "obs": c.get("estado_esperado") or c.get("composicion") or "-",
        })
    ...
    if infra_export == "csv":
        ...
        for r in infra_rows:
            writer.writerow([
                r["sede"], r["deposito"], r.get("piso", "-"), r.get("descripcion_inventario", "-"), r["personas_reales"], r.get("dependencia_detectada", "-"), r["fuero"],
                (f"{r['criterio_personal_codigo']} - {r['criterio_personal_nombre']}" if r.get("criterio_personal_codigo") else "-"),
    ...
    _set_override("S06", "D08", "menores")
    _set_range("S06", 9, 11, "penal")
    ...
    _set_override("S16", "D02", "menores")
    _set_override("S18", "D04", "menores")
    _set_override("S18", "D05", "menores")
    ...
    for sede, dep in sorted(all_keys_resumen, key=lambda x: (x[0], x[1])):
        ...
        if not fuero_local and sede == "S12":
            if dep in ("D04", "D09", "D24", "D25"):
                ...
                fuero_local = "administracion"
        # Regla fija S13: D36/D37/D38 = Administracion; resto = Menores.
        if not fuero_local and sede == "S13":
            if dep in ("D36", "D37", "D38"):
                fuero_local = "administracion"
            else:
                fuero_local = "menores"
        # S15: D03 menores; resto juridico social.
        if not fuero_local and sede == "S15":
            fuero_local = "menores" if dep == "D03" else "social"
        ...
        # S18: D04/D05 menores; resto juridico social.
        if not fuero_local and sede == "S18":
            fuero_local = "menores" if dep in ("D04", "D05") else "social"
        # S19/S20: juridico social puro.
        if not fuero_local and sede in ("S19", "S20"):
            fuero_local = "social"
    ...
    return render_template(
        ...
        infra_criterios_ref=infra_criterios_ref,
...
def _luminarias_operativo_table_rows(con, sede_codigo, piso, local_filter=""):
    ...
    for row in rows:
        ...
        items.append({
            ...
            "codigo_corto": str(row["codigo_corto"] or "").strip(),
            "codigo_completo": str(row["codigo_completo"] or "").strip(),
            "marca_actual": str(row["marca_actual"] or "").strip(),
            "supports_replacements": type_code != "PT",
            "detail_url": url_for(
                "sede_ficha",
                codigo=sede,
                piso=piso_norm,
...
def _luminaria_baja_history_detail(raw_reason):
    reason = str(raw_reason or "").strip()
    if not reason or reason.lower().startswith("baja confirmada por"):
        return "Eliminado"
    return reason


def _luminarias_history_feed(
    con,
    sede_codigo,
    piso_codigo,
    local_filter="",
    ...
    sede = (sede_codigo or "").upper().strip()
    piso = _mobiliario_plano_normalize_piso(piso_codigo)
    ambiente = _mobiliario_plano_normalize_local(ambiente_filtro or local_filter) if (ambiente_filtro or local_filter) else ""
    fecha_norm = (_luminaria_parse_date(fecha_filtro) or "").isoformat() if fecha_filtro else ""
    codigo_norm = str(codigo_filtro or "").strip().lower()
    movimiento = str(movimiento_filtro or "").strip().lower()
    if movimiento not in ("alta", "recambio", "baja"):
        movimiento = ""
    ...
    point_params = [sede, piso]
    if ambiente:
        point_where.append("p.ambiente_codigo = ?")
        point_params.append(ambiente)
    ...
    points = con.execute(f"""
        SELECT
            ...
            p.codigo_corto,
            p.codigo_completo,
            COALESCE(p.fecha_creacion,'') AS fecha_creacion,
            COALESCE(p.fecha_baja,'') AS fecha_baja,
            COALESCE(p.motivo_baja,'') AS motivo_baja,
            COALESCE(p.created_at,'') AS created_at,
            COALESCE(p.updated_at,'') AS updated_at
        FROM luminaria_punto p
        WHERE {" AND ".join(point_where)}
        ORDER BY p.id
    """, point_params).fetchall()
    point_by_id = {int(row["id"]): row for row in points}

    def code_matches(point_row):
        if not codigo_norm:
            return True
        short_code = str(point_row["codigo_corto"] or "").strip().lower()
        full_code = str(point_row["codigo_completo"] or "").strip().lower()
        return codigo_norm in short_code or codigo_norm in full_code

    def date_matches(value):
        if not fecha_norm:
            return True
        return str(value or "").strip() == fecha_norm

    def point_url(point_row):
        ambiente_codigo = str(point_row["ambiente_codigo"] or "").strip()
        return url_for(
            "sede_ficha",
            codigo=sede,
            piso=piso,
            local="" if ambiente_codigo == "SEDE" else ambiente_codigo,
            tab="luminarias",
            view="operativo",
            lview="operativo",
            lpoint=int(point_row["id"]),
        )

    events = []
    for point in points:
        if (not movimiento or movimiento == "alta") and point["fecha_creacion"] and code_matches(point) and date_matches(point["fecha_creacion"]):
            events.append({
                "fecha": str(point["fecha_creacion"] or "").strip(),
                "fecha_display": _luminaria_format_date(point["fecha_creacion"]),
                "movimiento": "Alta",
                "movimiento_key": "alta",
                "codigo_corto": str(point["codigo_corto"] or "").strip(),
                "codigo_completo": str(point["codigo_completo"] or "").strip(),
                "ambiente_codigo": str(point["ambiente_codigo"] or "").strip(),
                "detalle": str(point["ambiente_codigo"] or "").strip() or "-",
                "tipo": _luminaria_norm_type(point["tipo"]),
                "point_id": int(point["id"]),
                "point_url": point_url(point),
                "sort_rank": 2,
                "sort_ref": str(point["created_at"] or ""),
            })
        if (not movimiento or movimiento == "baja") and point["fecha_baja"] and code_matches(point) and date_matches(point["fecha_baja"]):
            events.append({
                "fecha": str(point["fecha_baja"] or "").strip(),
                "fecha_display": _luminaria_format_date(point["fecha_baja"]),
                "movimiento": "Baja",
                "movimiento_key": "baja",
                "codigo_corto": str(point["codigo_corto"] or "").strip(),
                "codigo_completo": str(point["codigo_completo"] or "").strip(),
                "ambiente_codigo": str(point["ambiente_codigo"] or "").strip(),
                "detalle": _luminaria_baja_history_detail(point["motivo_baja"]),
                "tipo": _luminaria_norm_type(point["tipo"]),
                "point_id": int(point["id"]),
                "point_url": point_url(point),
                "sort_rank": 1,
                "sort_ref": str(point["updated_at"] or ""),
            })

    recambio_rows = con.execute(f"""
        SELECT
            r.id,
            r.luminaria_punto_id,
            COALESCE(r.marca,'') AS marca,
            COALESCE(r.marca_otro,'') AS marca_otro,
            COALESCE(r.fecha_colocacion,'') AS fecha_colocacion,
            COALESCE(r.observacion,'') AS observacion,
            COALESCE(r.fecha_registro,'') AS fecha_registro
        FROM luminaria_recambio r
        JOIN luminaria_punto p ON p.id = r.luminaria_punto_id
        WHERE {" AND ".join(point_where)}
        ORDER BY COALESCE(r.fecha_colocacion,''), r.id
    """, point_params).fetchall()

    for row in recambio_rows:
        point = point_by_id.get(int(row["luminaria_punto_id"]))
        if not point or (movimiento and movimiento != "recambio"):
            continue
        if not row["fecha_colocacion"] or not code_matches(point) or not date_matches(row["fecha_colocacion"]):
            continue
        detail = _luminaria_brand_display(row)
        observacion = str(row["observacion"] or "").strip()
        if observacion:
            detail = f"{detail} · {observacion}" if detail else observacion
        events.append({
            "fecha": str(row["fecha_colocacion"] or "").strip(),
            "fecha_display": _luminaria_format_date(row["fecha_colocacion"]),
            "movimiento": "Recambio",
            "movimiento_key": "recambio",
            "codigo_corto": str(point["codigo_corto"] or "").strip(),
            "codigo_completo": str(point["codigo_completo"] or "").strip(),
            "ambiente_codigo": str(point["ambiente_codigo"] or "").strip(),
            "detalle": detail or "Sin marca",
            "tipo": _luminaria_norm_type(point["tipo"]),
            "point_id": int(point["id"]),
            "point_url": point_url(point),
            "sort_rank": 3,
            "sort_ref": f"{str(row['fecha_registro'] or '')}:{int(row['id'])}",
        })

    events.sort(key=lambda item: (item["fecha"], item["sort_rank"], item["sort_ref"]), reverse=True)
    return events
...
def luminarias_punto_editar(point_id):
    ...
    try:
        ...
        if not point:
            ...
            return redirect(url_for("dashboard"))
        if not bool(point["activo"]):
            flash("Solo puedes editar puntos activos.", "warning")
            return redirect(_luminaria_sede_return_url(point["sede_codigo"], point["piso_codigo"], point["ambiente_codigo"], point_id))

        ambiente_codigo = _mobiliario_plano_normalize_local(request.form.get("ambiente_codigo") or point["ambiente_codigo"] or "")
        fecha_creacion = (request.form.get("fecha_creacion") or point["fecha_creacion"] or "").strip()
        nuevo_tipo = _luminaria_norm_type(request.form.get("tipo") or point["tipo"])
        ambiente_options = _luminaria_environment_options(con, point["sede_codigo"], point["piso_codigo"])
        ambiente_codes = {item["codigo"] for item in ambiente_options}
        if ambiente_codigo not in ambiente_codes:
            flash("Selecciona un ambiente valido del piso actual.", "warning")
            return redirect(_luminaria_sede_return_url(point["sede_codigo"], point["piso_codigo"], point["ambiente_codigo"], point_id))

        fecha_alta = _luminaria_parse_date(fecha_creacion)
        if not fecha_alta:
            flash("La fecha del punto no es valida.", "warning")
            return redirect(_luminaria_sede_return_url(point["sede_codigo"], point["piso_codigo"], point["ambiente_codigo"], point_id))

        first_history = con.execute("""
            SELECT MIN(fecha_colocacion) AS fecha_min
            FROM luminaria_recambio
            WHERE luminaria_punto_id = ?
        """, (int(point_id),)).fetchone()
        first_history_date = _luminaria_parse_date(first_history["fecha_min"]) if first_history and first_history["fecha_min"] else None
        if first_history_date and fecha_alta > first_history_date:
            flash("La fecha del punto no puede ser posterior al primer registro de recambio.", "warning")
            return redirect(_luminaria_sede_return_url(point["sede_codigo"], point["piso_codigo"], point["ambiente_codigo"], point_id))

        fecha_baja = _luminaria_parse_date(point["fecha_baja"])
        if fecha_baja and fecha_alta > fecha_baja:
            flash("La fecha del punto no puede ser posterior a la baja registrada.", "warning")
            return redirect(_luminaria_sede_return_url(point["sede_codigo"], point["piso_codigo"], point["ambiente_codigo"], point_id))

        tipo_actual = _luminaria_norm_type(point["tipo"])
        if nuevo_tipo != tipo_actual:
            ...
            """, (int(point_id),)).fetchone()
            if has_history:
                flash("El tipo solo puede cambiarse si el punto todavia no tiene historial.", "warning")
                return redirect(_luminaria_sede_return_url(point["sede_codigo"], point["piso_codigo"], point["ambiente_codigo"], point_id))
            duplicated_type = con.execute("""
                SELECT id
                FROM luminaria_punto
                WHERE UPPER(COALESCE(sede_codigo,'')) = ?
                  AND tipo = ?
                ...
                LIMIT 1
            """, (
                str(point["sede_codigo"] or "").upper().strip(),
                nuevo_tipo,
                int(point["numero"] or 0),
                int(point_id),
            )).fetchone()
            if duplicated_type:
                flash("No se puede cambiar el tipo porque ese codigo ya existe en la sede.", "warning")
                return redirect(_luminaria_sede_return_url(point["sede_codigo"], point["piso_codigo"], point["ambiente_codigo"], point_id))

        codigo_corto = _luminaria_short_code(nuevo_tipo, point["numero"])
        codigo_completo = _luminaria_full_code(point["sede_codigo"], ambiente_codigo, nuevo_tipo, point["numero"])
        duplicated_code = con.execute("""
            SELECT id
            FROM luminaria_punto
            WHERE codigo_completo = ?
              AND id <> ?
            LIMIT 1
        """, (codigo_completo, int(point_id))).fetchone()
        if duplicated_code:
            flash("No se pudo guardar porque ese codigo ya existe.", "warning")
            return redirect(_luminaria_sede_return_url(point["sede_codigo"], point["piso_codigo"], point["ambiente_codigo"], point_id))
        ...
        old_ambiente = _mobiliario_plano_normalize_local(point["ambiente_codigo"])
        _luminaria_sync_summary_for_ambiente(con, point["sede_codigo"], point["piso_codigo"], old_ambiente)
        if ambiente_codigo != old_ambiente:
            _luminaria_sync_summary_for_ambiente(con, point["sede_codigo"], point["piso_codigo"], ambiente_codigo)
        con.commit()
        flash("Punto actualizado.", "success")
        return redirect(_luminaria_sede_return_url(point["sede_codigo"], point["piso_codigo"], ambiente_codigo, point_id))
    finally:
        con.close()
